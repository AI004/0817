...
Estimation_Operator::~Estimation_Operator() { delete q_dot_a_filter; }
...
void Estimation_Operator::init(Robot_Data *robotdata) {
  ...
  momentum_filter = new LowPassFilter(13, 0.707, robotdata->dt, 6);
  stateestimator = new WalkStateEstimate();
  momentumController = new MomentumController();
}
// to be delete
void Estimation_Operator::task_state_update(Robot_Data *robotdata) {
  ...
  RigidBodyDynamics::UpdateKinematicsCustom(*(robotdata->robot_model),
                                            &(robotdata->q_a),
                                            &(robotdata->q_dot_a), nullptr);
  ...
  for (std::vector<Task *>::iterator iter = robotdata->task_card_set.begin();
    ...
    case general_task: {
      ...
      Eigen::Vector3d pa;
      pa = RigidBodyDynamics::CalcBodyToBaseCoordinates(
          *(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
          (*iter)->T_offset.block(0, 3, 3, 1), false);
      Eigen::Matrix3d Ra;
      // Eigen::Vector3d Euler_ZYX;
      Eigen::Vector3d Euler_XYZ;
      Ra = RigidBodyDynamics::CalcBodyWorldOrientation(
               *(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
               false)
               .transpose();
      ...
      RigidBodyDynamics::CalcPointJacobian6D(
          *(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
          (*iter)->T_offset.block(0, 3, 3, 1), J_6D, false);
      ...
      RigidBodyDynamics::Math::VectorNd Jdotqdot_6D =
          RigidBodyDynamics::CalcPointAcceleration6D(
              *(robotdata->robot_model), robotdata->q_a, robotdata->q_dot_a,
              RigidBodyDynamics::Math::VectorNd::Zero(robotdata->ndof),
              (*iter)->joint_id, (*iter)->T_offset.block(0, 3, 3, 1), false);
      Jdotqdot_6D.block(3, 0, 3, 1) =
          Jdotqdot_6D.block(3, 0, 3, 1) + robotdata->robot_model->gravity;
      //            (*iter)->jacobi_dot_q_dot.setZero((*iter)->dim,robotdata->ndof);
      // select the right direction
      for (uint i = 0; i < (*iter)->task_selection_matrix.size(); i++) {
        (*iter)->jacobi_dot_q_dot.row(i) =
            Jdotqdot_6D.row((*iter)->task_selection_matrix[i]);
      }
      // update x_a velocity
      (*iter)->X_a.row(1) = ((*iter)->jacobi * robotdata->q_dot_a).transpose();
      // update x_a acc
      (*iter)->X_a.row(2) =
          ((*iter)->jacobi * robotdata->q_ddot_a + (*iter)->jacobi_dot_q_dot)
              .transpose();
      // std::cout<<"jacobi:"<< std::endl<<(*iter)->jacobi<<std::endl;
      // std::cout<<"q_ddot_a:"<<
      // std::endl<<robotdata->q_ddot_a.transpose()<<std::endl;
      // std::cout<<"jacobi_dot_q_dot:"<<
      // std::endl<<(*iter)->jacobi_dot_q_dot.transpose()<<std::endl; update F:
      // if the task has no sensor , F will  keep zero compute controller value
      (*iter)->controller->setinput_data((*iter)->X_a, (*iter)->X_d,
                                         (*iter)->dim, robotdata->dt);
    ...
    case com_task: {
      ...
      Eigen::Vector3d Euler_XYZ;
      Ra_F2I = RigidBodyDynamics::CalcBodyWorldOrientation(
                   *(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
                   false)
                   .transpose();
      ...
      Eigen::Vector3d _IP1;
      Eigen::Vector3d _IPG;
      _IP1 = RigidBodyDynamics::CalcBodyToBaseCoordinates(
          *(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
          (*iter)->T_offset.block(0, 3, 3, 1), false);
      ...
      _1XTG.block(0, 3, 3, 3) =
          PH1.block(0, 3, 3, 3) * (basicfunction::skew(_1PG)).transpose();
      // AG
      ...
      RigidBodyDynamics::NonlinearEffects(*(robotdata->robot_model), _q_a,
                                          _q_dot_a, CG);
      RigidBodyDynamics::NonlinearEffects(*(robotdata->robot_model), _q_a,
                                          QDotZero, G);
      C = CG - G;
      ...
      for (uint i = 0; i < (*iter)->task_selection_matrix.size(); i++) {
        (*iter)->jacobi_dot_q_dot(i) =
            AGdotqdot((*iter)->task_selection_matrix[i]);
      }
      // update x_a velocity
      (*iter)->X_a.row(1) = ((*iter)->jacobi * robotdata->q_dot_a).transpose();
      // update x_a acc
      ...
      //     (*iter)->X_c.row(2).block(0,0,1,3) =
      //     (*iter)->X_d.row(2).block(0,0,1,3) +
      //     (*iter)->controller->para(1)*((*iter)->X_d.row(1).block(0,0,1,3) -
      //     (*iter)->X_a.row(1).block(0,0,1,3));
      // }
      (*iter)->X_c.row(2)(0) =
          (*iter)->X_d.row(2)(0) +
          para_(6) * ((*iter)->X_d.row(1)(0) - (*iter)->X_a.row(1)(0));
      (*iter)->X_c.row(2)(1) =
          (*iter)->X_d.row(2)(1) +
          para_(7) * ((*iter)->X_d.row(1)(1) - (*iter)->X_a.row(1)(1));
      (*iter)->X_c.row(2)(2) =
          (*iter)->X_d.row(2)(2) +
          para_(8) * ((*iter)->X_d.row(1)(2) - (*iter)->X_a.row(1)(2));
      // linear moment control
      // (*iter)->X_c.row(2).block(0,3,1,3) = (*iter)->X_d.row(2).block(0,3,1,3)
      // + (*iter)->controller->para(2)*((*iter)->X_d.row(1).block(0,3,1,3) -
      // (*iter)->X_a.row(1).block(0,3,1,3))
      // + M * (*iter)->controller->para(0)*((*iter)->X_d.row(0).block(0,3,1,3)
      // - (*iter)->X_a.row(0).block(0,3,1,3));
      (*iter)->X_c.row(2)(3) =
          (*iter)->X_d.row(2)(3) +
          para_(9) * ((*iter)->X_d.row(1)(3) - (*iter)->X_a.row(1)(3)) +
          M * para_(3) * ((*iter)->X_d.row(0)(3) - (*iter)->X_a.row(0)(3));
      (*iter)->X_c.row(2)(4) =
          (*iter)->X_d.row(2)(4) +
          para_(10) * ((*iter)->X_d.row(1)(4) - (*iter)->X_a.row(1)(4)) +
          M * para_(4) * ((*iter)->X_d.row(0)(4) - (*iter)->X_a.row(0)(4));
      (*iter)->X_c.row(2)(5) =
          (*iter)->X_d.row(2)(5) +
          para_(11) * ((*iter)->X_d.row(1)(5) - (*iter)->X_a.row(1)(5)) +
          M * para_(5) * ((*iter)->X_d.row(0)(5) - (*iter)->X_a.row(0)(5));

      // transform to momentum
      // simple model
      // (*iter)->X_c.row(2).block(0,3,1,3) =
      // M*(*iter)->X_c.row(2).block(0,3,1,3); whole model
      // (*iter)->X_c.row(2) =
      // (I1C*((*iter)->X_c.row(2).transpose())).transpose();

      // y linear direction reduce
      // (*iter)->X_c.row(2).block(0,4,1,1) =
      // 0.5*(*iter)->X_c.row(2).block(0,4,1,1);
      // (*iter)->controller->getoutput_data((*iter)->X_c);
    ...
    case contact_task: {
      ...
      if ((*iter)->sensor_id != 0) {
        int id = 0;
        for (uint sensor_i = 0; sensor_i < robotdata->sensor_set.size();
             sensor_i++) {
        ...
        case FT_sensor: {
          if (robotdata->sensor_set[id]->link_num != (*iter)->joint_id) {
            std::cout << "Warning: FT_sensor link num does not match the task "
                         "joint_id!"
                      << std::endl;
          } else {
            Eigen::Matrix<double, 6, 1> wrench =
                robotdata->sensor_set[id]->_data;
            RigidBodyDynamics::Math::SpatialTransform X_sensortolink(
                robotdata->sensor_set[id]->T_offset.block(0, 0, 3, 3),
                -robotdata->sensor_set[id]
                        ->T_offset.block(0, 0, 3, 3)
                        .transpose() *
                    robotdata->sensor_set[id]->T_offset.block(0, 3, 3, 1));
            RigidBodyDynamics::Math::SpatialTransform X_tasktolink(
                (*iter)->T_offset.block(0, 0, 3, 3),
                -(*iter)->T_offset.block(0, 0, 3, 3).transpose() *
                    ((*iter)->T_offset.block(0, 3, 3, 1)));
            RigidBodyDynamics::Math::SpatialTransform X_sensortotask =
                X_sensortolink * (X_tasktolink.inverse());
        ...
        case Joint_external_torque_observer: {
          Eigen::MatrixXd jacobi_T_inv =
              ((*iter)->jacobi * (*iter)->jacobi.transpose())
                  .completeOrthogonalDecomposition()
                  .pseudoInverse() *
              (*iter)->jacobi;
          (*iter)->X_a.row(3) =
              (jacobi_T_inv * robotdata->sensor_set[id]->_data).transpose();
          //                        std::cout<<"(*iter)->X_a.row(3):
          //                        "<<(*iter)->X_a.row(3)<<std::endl;
          break;
        }
        default: {
      ...
      robotdata->tau_ext =
          robotdata->tau_ext -
          (*iter)->jacobi.transpose() * (*iter)->X_a.row(3).transpose();
      // compute controller value
      (*iter)->controller->setinput_data((*iter)->X_a, (*iter)->X_d,
                                         (*iter)->dim, robotdata->dt);
      (*iter)->controller->controller_run();
    ...
    case joint_task: {
      if (robotdata->robottype == robot_type::Fixed_Base_Open_Chain) {
        ...
        if (robotdata->wbcsolver == Wbc_Solver_type::hqp_dynamics) {
          (*iter)->jacobi =
              (*iter)->jacobi * sqrt(1.0 + robotdata->dt * robotdata->dt);
          if (robotdata->robottype == robot_type::Mobile_Wheel_Open_Chain) {
            (*iter)->jacobi.block(0, start_row, robotdata->ndof_wheel,
                                  robotdata->ndof_wheel) =
                Eigen::MatrixXd::Identity(robotdata->ndof_wheel,
                                          robotdata->ndof_wheel) *
                robotdata->dt;
          }
        }
        ...
        // update x_a position velocity acc
        (*iter)->X_a.row(0) =
            robotdata->q_a.block(start_row, 0, _dim, 1).transpose();
        (*iter)->X_a.row(1) =
            robotdata->q_dot_a.block(start_row, 0, _dim, 1).transpose();
        (*iter)->X_a.row(2) =
            robotdata->q_ddot_a.block(start_row, 0, _dim, 1).transpose();
        // update F
        (*iter)->X_a.row(3) =
            robotdata->tau_ext.block(start_row, 0, _dim, 1).transpose();
      }
      ...
      //            if((*iter)->sensor_id != 0)
      //            {
      //                if(robotdata->sensor_set[(*iter)->sensor_id]->type ==
      //                Joint_external_torque_observer)
      //                {
      //                   (*iter)->X_a.row(3) = robotdata->tau_ext.transpose();
      //                }else{
      //                    (*iter)->X_a.row(3) =
      //                    robotdata->tau_ext.transpose();
      //                }
      //            }else{
      //                (*iter)->X_a.row(3) = robotdata->tau_ext.transpose();
      //            }
      ...
      if (robotdata->wbcsolver == Wbc_Solver_type::hqp_dynamics) {
        ...
        (*iter)->X_c.row(2) =
            (*iter)->X_c.row(2) + (*iter)->X_a.row(1) * robotdata->dt;
      }
      // end
...
void Estimation_Operator::externaltorqueobserver(Robot_Data *robotdata) {
  // H
  RigidBodyDynamics::Math::MatrixNd H =
      RigidBodyDynamics::Math::MatrixNd::Zero(robotdata->ndof, robotdata->ndof);
  RigidBodyDynamics::Math::VectorNd _q_a = robotdata->q_a;
  RigidBodyDynamics::Math::VectorNd _q_dot_a = robotdata->q_dot_a;
  ...
  RigidBodyDynamics::NonlinearEffects(*(robotdata->robot_model), _q_a, _q_dot_a,
                                      CG);
#ifdef DEBUG
...
void Estimation_Operator::task_state_update_x_a(Robot_Data *robotdata) {
  ...
  //     //          // update jacobi_dotq_dot
  //     //             RigidBodyDynamics::Math::VectorNd Jdotqdot_6D =
  //     //             RigidBodyDynamics::CalcPointAcceleration6D(*(robotdata->robot_model),robotdata->q_a,
  //     //                                                        robotdata->q_dot_a,RigidBodyDynamics::Math::VectorNd::Zero(robotdata->ndof),
  //     //                                                        (*iter)->joint_id,(*iter)->T_offset.block(0,3,3,1),false);
  //     //             Jdotqdot_6D.block(3,0,3,1) = Jdotqdot_6D.block(3,0,3,1) +
  //     //             robotdata->robot_model->gravity;
  //     // // (*iter)->jacobi_dot_q_dot.setZero((*iter)->dim,robotdata->ndof);
  //     //             // select the right direction
  //     //             for(uint i = 0; i <
  //     //             (*iter)->task_selection_matrix.size();i++)
  //     //             {
  //     //                 (*iter)->jacobi_dot_q_dot.row(i) =
  //     //                 Jdotqdot_6D.row((*iter)->task_selection_matrix[i]);
  //     //             }
  ...
  //     //  std::cout<<"q_dot_a:
  //     //  "<<std::endl<<robotdata->q_dot_a.transpose()<<std::endl;
  //     (*iter)->X_a.row(1) = ((*iter)->jacobi * robotdata->q_dot_a).transpose();
  //     // update x_a acc
  //     // (*iter)->X_a.row(2) = ((*iter)->jacobi*robotdata->q_ddot_a +
  //     // (*iter)->jacobi_dot_q_dot).transpose(); std::cout<<"jacobi:"<<
  //     // std::endl<<(*iter)->jacobi<<std::endl; std::cout<<"q_ddot_a:"<<
  //     // std::endl<<robotdata->q_ddot_a.transpose()<<std::endl;
  //     // std::cout<<"jacobi_dot_q_dot:"<<
  //     // std::endl<<(*iter)->jacobi_dot_q_dot.transpose()<<std::endl;
  //     // std::cout<<"X_a: "<<(*iter)->X_a<<std::endl;
  //     // std::cout<<"X_d: "<<(*iter)->X_d<<std::endl;
  //     // std::cout<<"X_c: "<<(*iter)->X_c<<std::endl;
  //     break;
  ...
  //     // update x_a acc
  //     // (*iter)->X_a.row(2) = ((*iter)->jacobi*robotdata->q_ddot_a +
  //     // (*iter)->jacobi_dot_q_dot).transpose(); update F: if the task has no
  //     // sensor , F will  keep zero
  //     // (*iter)->X_a.row(3).setZero();
  ...
  //     // update x_a acc
  //     // (*iter)->X_a.row(2) = ((*iter)->jacobi*robotdata->q_ddot_a +
  //     // (*iter)->jacobi_dot_q_dot).transpose(); std::cout<<"X_a:
  //     // "<<(*iter)->X_a<<std::endl; std::cout<<"X_d:
  //     // "<<(*iter)->X_d<<std::endl; std::cout<<"X_c:
  //     // "<<(*iter)->X_c<<std::endl;
  //     break;
  //   }
  //   default: {
  ...
  stateestimator->estWaistPosVelInWorld(robotdata, robotdata->stance_index);
  ...
  for (std::vector<Task *>::iterator iter = robotdata->task_card_set.begin();
    ...
    case com_task: {
      ...
      RigidBodyDynamics::CompositeRigidBodyAlgorithm(*(robotdata->robot_model),
                                                     _q_a, H, false);
      // define U1
      Eigen::MatrixXd U1 = Eigen::MatrixXd::Zero(6, robotdata->ndof);
      U1.block(0, 0, 6, 6) = Eigen::MatrixXd::Identity(6, 6);
      // H11
      Eigen::MatrixXd H11 = U1 * H * U1.transpose();
      // I_1^C
      ...
      _IPG = _IP1 + Ra_F2I * _1PG;
      // _1XTG
      Eigen::MatrixXd _1XTG = Eigen::MatrixXd::Zero(6, 6);
      _1XTG.block(0, 0, 3, 3) = PH1.block(0, 3, 3, 3);
      _1XTG.block(3, 3, 3, 3) = PH1.block(3, 0, 3, 3);
      _1XTG.block(0, 3, 3, 3) =
          PH1.block(0, 3, 3, 3) * (basicfunction::skew(_1PG)).transpose();
      ...
      RigidBodyDynamics::NonlinearEffects(*(robotdata->robot_model), _q_a,
                                          QDotZero, G);
      C = CG - G;
      Eigen::MatrixXd AGdotqdot = _1XTG * PS1.transpose() * U1 * C * _q_dot_a;
      // I_G^3
      ...
      Eigen::Matrix<double, 1, 6> momentum;
      momentum = momentum_filter->mFilter(
          ((*iter)->jacobi * robotdata->q_dot_a).transpose());

      // update x_a velocity
      (*iter)->X_a.row(1) = momentum;
      // update x_a acc
      (*iter)->X_a.row(2) =
          ((*iter)->jacobi * robotdata->q_ddot_a + (*iter)->jacobi_dot_q_dot)
              .transpose();
...
void Estimation_Operator::task_state_update_x_a_walk(Robot_Data *robotdata) {
    ...
    case com_task: {
      ...
      Ra_F2I = RigidBodyDynamics::CalcBodyWorldOrientation(
                   *(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
                   false)
                   .transpose();
      basicfunction::matrixtoeulerxyz_(Ra_F2I, Euler_XYZ);
      // compute jacobi
      Eigen::MatrixXd J_6D;
      J_6D.setZero(6, robotdata->ndof);
      RigidBodyDynamics::CalcPointJacobian6D(
  ...
  stateestimator->grfEstimating(robotdata);
  // compute stance phases
  stateestimator->generalStateMachine(robotdata);
  stateestimator->estWaistPosVelInWorld(robotdata);
  ...
  for (std::vector<Task *>::iterator iter = robotdata->task_card_set.begin();
    ...
    case com_task: {
      ...
      (*iter)->X_a.row(3).setZero();
      robotdata->v_com =
          ((*iter)->X_a.row(1).tail(3).transpose() +
           (*iter)->X_a.row(1).head(3).transpose() / (*iter)->X_a(0, 5)) /
          M;
      // std::cout<<"X_a(com): "<<std::endl<<(*iter)->X_a<<std::endl;
      // std::cout<<"X_d(com): "<<(*iter)->X_d<<std::endl;
      // std::cout<<"X_c(com): "<<(*iter)->X_c<<std::endl;
...
void Estimation_Operator::task_state_update_x_d(Robot_Data *robotdata) {
  ...
  for (std::vector<Task *>::iterator iter = robotdata->task_card_set.begin();
    ...
    case com_task: {
      ...
      // robotdata->net_contact_force =
      // (*iter)->IG*((*iter)->X_c.row(2).transpose() - bias);

      momentumController->setInputData((*iter)->X_a, (*iter)->X_d,
                                       (*iter)->controller->para, (*iter)->IG);
      momentumController->controllerRun(robotdata);
      momentumController->getOutputData((*iter)->X_c);

      robotdata->net_contact_force = (*iter)->X_c.row(2).transpose() - bias;
      // std::cout << "X_a(com): " << std::endl << (*iter)->X_a << std::endl;
      // std::cout << "X_d(com): " << std::endl << (*iter)->X_d << std::endl;
      // std::cout << "X_c(com): " << std::endl << (*iter)->X_c << std::endl;
      // std::cout << "GRF: " << std::endl << robotdata->grf << std::endl;
      // std::cout<<"net_contact_force:
      // "<<std::endl<<robotdata->net_contact_force<<std::endl; std::cout<<"G:
      // "<<std::endl<<robotdata->G<<std::endl;
      break;
    }
    case contact_task: {
      // compute controller value
  ...
  for (std::vector<Task *>::iterator iter = robotdata->task_card_set.begin();
    ...
    case joint_task: {
      ...
      // end
      // std::cout<<"X_a(joint): "<<std::endl<<(*iter)->X_a<<std::endl;
      // std::cout<<"X_d(joint): "<<std::endl<<(*iter)->X_d<<std::endl;
      // std::cout<<"X_c(joint): "<<std::endl<<(*iter)->X_c<<std::endl;
      break;
    }
    default:
      break;
