...
void Estimation_Operator::task_state_update(Robot_Data* robotdata) {
  ...
  RigidBodyDynamics::UpdateKinematicsCustom(*(robotdata->robot_model), &(robotdata->q_a), &(robotdata->q_dot_a),
                                            nullptr);
  // external torque observer
  externaltorqueobserver(robotdata);

  // update task card not for joint task
  for (std::vector<Task*>::iterator iter = robotdata->task_card_set.begin(); iter != robotdata->task_card_set.end();
       iter++) {
    switch ((*iter)->type) {
      case general_task: {
        ...
        Eigen::Vector3d pa;
        pa = RigidBodyDynamics::CalcBodyToBaseCoordinates(*(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
                                                          (*iter)->T_offset.block(0, 3, 3, 1), false);
        Eigen::Matrix3d Ra;
        // Eigen::Vector3d Euler_ZYX;
        Eigen::Vector3d Euler_XYZ;
        Ra = RigidBodyDynamics::CalcBodyWorldOrientation(*(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
                                                         false)
                 .transpose();
        ...
        RigidBodyDynamics::CalcPointJacobian6D(*(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id,
      ...
      case com_task: {
        ...
        _IP1 = RigidBodyDynamics::CalcBodyToBaseCoordinates(
            *(robotdata->robot_model), robotdata->q_a, (*iter)->joint_id, (*iter)->T_offset.block(0, 3, 3, 1), false);
        ...
        (*iter)->X_c.row(2)(3) = (*iter)->X_d.row(2)(3) + para_(9) * ((*iter)->X_d.row(1)(3) - (*iter)->X_a.row(1)(3)) +
                                 M * para_(3) * ((*iter)->X_d.row(0)(3) - (*iter)->X_a.row(0)(3));
        (*iter)->X_c.row(2)(4) = (*iter)->X_d.row(2)(4) +
                                 para_(10) * ((*iter)->X_d.row(1)(4) - (*iter)->X_a.row(1)(4)) +
                                 M * para_(4) * ((*iter)->X_d.row(0)(4) - (*iter)->X_a.row(0)(4));
        (*iter)->X_c.row(2)(5) = (*iter)->X_d.row(2)(5) +
          ...
          switch (robotdata->sensor_set[id]->type) {
            case FT_sensor: {
              if (robotdata->sensor_set[id]->link_num != (*iter)->joint_id) {
                ...
                RigidBodyDynamics::Math::SpatialTransform X_sensortolink(
                    ...
                    -robotdata->sensor_set[id]->T_offset.block(0, 0, 3, 3).transpose() *
            ...
            case Joint_external_torque_observer: {
              Eigen::MatrixXd jacobi_T_inv =
                  ((*iter)->jacobi * (*iter)->jacobi.transpose()).completeOrthogonalDecomposition().pseudoInverse() *
                  (*iter)->jacobi;
        ...
        if (robotdata->robottype == robot_type::Fixed_Base_Open_Chain) {
          ...
          if (robotdata->wbcsolver == Wbc_Solver_type::hqp_dynamics) {
            ...
            if (robotdata->robottype == robot_type::Mobile_Wheel_Open_Chain) {
              (*iter)->jacobi.block(0, start_row, robotdata->ndof_wheel, robotdata->ndof_wheel) =
                  Eigen::MatrixXd::Identity(robotdata->ndof_wheel, robotdata->ndof_wheel) * robotdata->dt;
            }
          }
          // end
          // update jacobi_dot
...
void Estimation_Operator::task_state_update_x_a(Robot_Data* robotdata) {
  ...
  stateestimator->estCOMStateMomentum(robotdata);
  // external torque observer
  // externaltorqueobserver(robotdata);
  ...
  for (std::vector<Task*>::iterator iter = robotdata->task_card_set.begin(); iter != robotdata->task_card_set.end();
    ...
    switch ((*iter)->type) {
      ...
      case com_task: {
        ...
        G.setZero(robotdata->ndof);
        //                Eigen::VectorXd C;
        RigidBodyDynamics::Math::VectorNd C = RigidBodyDynamics::Math::VectorNd::Zero(robotdata->ndof);
        //                Eigen::MatrixXd QDotZero = robotdata->q_dot_a;
        RigidBodyDynamics::Math::VectorNd QDotZero = robotdata->q_dot_a;
        QDotZero.setZero();
        _q_a = robotdata->q_a;
        ...
        momentum = momentum_filter->mFilter(((*iter)->jacobi * robotdata->q_dot_a).transpose());
  ...
  for (std::vector<Task*>::iterator iter = robotdata->task_card_set.begin(); iter != robotdata->task_card_set.end();
    ...
    switch ((*iter)->type) {
      ...
      case com_task: {
        ...
        (*iter)->X_a.row(3).setZero();
        robotdata->v_com =
            ((*iter)->X_a.row(1).tail(3).transpose() + (*iter)->X_a.row(1).head(3).transpose() / (*iter)->X_a(0, 5)) /
            M;
        // std::cout<<"X_a(com): "<<std::endl<<(*iter)->X_a<<std::endl;
        // std::cout<<"X_d(com): "<<(*iter)->X_d<<std::endl;
        // std::cout<<"X_c(com): "<<(*iter)->X_c<<std::endl;
  ...
  for (std::vector<Task*>::iterator iter = robotdata->task_card_set.begin(); iter != robotdata->task_card_set.end();
    ...
    switch ((*iter)->type) {
      ...
      case com_task: {
        ...
        momentumController->setInputData((*iter)->X_a, (*iter)->X_d, (*iter)->controller->para, (*iter)->IG);
