 ...
 */
enum robot_type { Fixed_Base_Open_Chain = 1, Float_Base_Open_Chain, Mobile_Wheel_Open_Chain };
/**
...
class Robot_Data {
 public:
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_a;       // current floating base and joints pos
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_dot_a;   // current floating base and joints vel
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_ddot_a;  // current floating base and joints acc
  Eigen::Matrix<double, Eigen::Dynamic, 1> tau_a;     // current floating base and joints torque
  // robot desired quantity
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_d;       // desired floating base and joints pos
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_dot_d;   // desired floating base and joints vel
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_ddot_d;  // desired floating base and joints acc
  Eigen::Matrix<double, Eigen::Dynamic, 1> tau_d;     // desired floating base and joints torque
  // robot command quantity
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_c;       // floating base and joints pos command
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_dot_c;   // floating base and joints vel command
  Eigen::Matrix<double, Eigen::Dynamic, 1> q_ddot_c;  // floating base and joints acc command
  Eigen::Matrix<double, Eigen::Dynamic, 1> tau_c;     // floating base and joints torque command
  // robot system paras
  ...
  double Tc_leg{0.02};
  int phase = 1;      // 1: right foot stance, 2: right double stance, 3: right flight, 4:
                      // left foot stance, 5: left double stance, 6: left flight
  int phase_pre = 1;  // 1: right foot stance, 2: right double stance, 3: right flight, 4:
                      // left foot stance, 5: left double stance, 6: left flight
  Eigen::Vector2d t0_leg = Eigen::Vector2d::Zero();        // start time of every gait cycle, gait circle
                                                           // contains stance, swing sequentially, so it's
                                                           // the time starts swing.
  Eigen::Vector2d offset_leg = Eigen::Vector2d::Zero();    // offset of every gait cycle, initial t0_leg =
                                                           // 0
                                                           // - offset_leg
  Eigen::Vector2d t_leg = Eigen::Vector2d::Zero();         // time in current gait cycle, t_leg = time -
                                                           // t0_leg
  ...
  Eigen::Vector2d st_leg_pre = Eigen::Vector2d::Ones();    // previous flag for stance index
  Eigen::Vector2d late_leg = Eigen::Vector2d::Zero();      // flag for late touch, if late_flag==1, this
  ...
  Eigen::Vector2d lx_leg = Eigen::Vector2d::Zero();        // x foothold location relative to waist
  Eigen::Vector2d ly_leg = Eigen::Vector2d::Zero();        // y foothold location relative to waist
  Eigen::VectorXd l_leg = Eigen::VectorXd::Zero(6);        // foothold location relative to waist
  Eigen::VectorXd v0_leg = Eigen::VectorXd::Zero(6);       // inital velocity for each stance phase
  Eigen::Vector2d vtd_leg = Eigen::Vector2d::Zero();       // predicted waist speed at touchdown
  Eigen::Vector2d vtd_filt_leg = Eigen::Vector2d::Zero();  // filted predicted waist speed at touchdown
  Eigen::Vector2d ptd_leg = Eigen::Vector2d::Zero();       // predicted waist position at touchdown
  double lambda;
  int flag_T = 1;
  Eigen::Vector2d isAnkleTouch = Eigen::Vector2d::Zero();
  Eigen::Vector2d isFootTouch = Eigen::Vector2d::Zero();

  Eigen::VectorXd pBodyf_a = Eigen::VectorXd::Zero(6);
  Eigen::VectorXd vBodyf_a = Eigen::VectorXd::Zero(6);
  Eigen::VectorXd pFootb_a = Eigen::VectorXd::Zero(6);
  Eigen::VectorXd vFootb_a = Eigen::VectorXd::Zero(6);
  ...
  Eigen::MatrixXd rFoot_ref = Eigen::MatrixXd::Zero(6, 3);  // foot reference pose
  ...
  bool onRun = false;
  // uniPlan
  Eigen::VectorXd imu9D = Eigen::VectorXd::Zero(9);
  ...
  Eigen::VectorXd tau_lb = Eigen::VectorXd::Zero(actor_num);  // wbc joint torque lower bound
  Eigen::VectorXd tau_ub = Eigen::VectorXd::Zero(actor_num);  // wbc joint torque upper bound
  Eigen::VectorXd GRF_ub = Eigen::VectorXd::Zero(12);         // wbc ground reaction force lower bound
  Eigen::VectorXd GRF_lb = Eigen::VectorXd::Zero(12);         // wbc ground reaction force upper bound
  ...
  bool use_IMU_correction = false;

  // use am calc foot hold
  double use_AM, l_start_offset, r_start_offset, forward_offset;
  bool use_AM_flag = false;
  double momentum_factor = 1.0;
  // com momentum
  Eigen::VectorXd comMomentum = Eigen::VectorXd::Zero(6);
  Eigen::VectorXd comMomentum_filt = Eigen::VectorXd::Zero(6);
