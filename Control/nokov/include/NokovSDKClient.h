...
// NOKOVSDKCLIENT_EXPORTS symbol defined on the command line. This symbol should
// not be defined on any project that uses this DLL. This way any other project
// whose source files include this file see NOKOVSDKCLIENT_API functions as
...
class NOKOVSDKCLIENT_API NokovSDKClient {
 public:
  ...
  int SetAnalogChCallback(void (*CallbackFunc)(sFrameOfAnalogChannelData* pAnChData, void* pUserData),
                          void* pUserData = 0);
  ...
  bool DecodeTimecode(unsigned int inTimecode, unsigned int inTimecodeSubframe, int* hour, int* minute, int* second,
  ...
  int GetLastFrameMarkerByName(const char* markersetName, const char* markerName,
                               float** data);  // markerset
  int GetLastFrameRigidBodyByName(const char* name,
                                  sRigidBodyData** data);  // 刚体
  int GetLastFrameSkeletonByName(const char* markerSetName, const char* skeName,
                                 sRigidBodyData** data);        // 骨骼
