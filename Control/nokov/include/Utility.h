...
class CalculateMethod {
 public:
  ...
  int tryToCalculate(IN Point* point, OUT ReferenceType variable) {
    if (nullptr == point) return 1;

    m_Points = point;

    if (m_FPS <= 0 || m_FPS >= 400) return 1;
...
class CalculateAcceleration : public CalculateMethod<Accel> {
 ...
 protected:
  virtual int calculate(OUT Accel& accel) override {
    ...
    accel.Ax = ((m_Points[TR * 2].x - m_Points[TR].x) * m_FPS / diffFrame1 -
                (m_Points[TR].x - m_Points[0].x) * m_FPS / diffFrame0) *
               m_FPS / diffFrame1;
    accel.Ay = ((m_Points[TR * 2].y - m_Points[TR].y) * m_FPS / diffFrame1 -
                (m_Points[TR].y - m_Points[0].y) * m_FPS / diffFrame0) *
               m_FPS / diffFrame1;
    accel.Az = ((m_Points[TR * 2].z - m_Points[TR].z) * m_FPS / diffFrame1 -
                (m_Points[TR].z - m_Points[0].z) * m_FPS / diffFrame0) *
               m_FPS / diffFrame1;
...
class SlideFrameArray {
 public:
  explicit SlideFrameArray() { clear(); }
 ...
 private:
  ...
  bool frameArray(OUT Point*& retArray, int frameFactor = FrameFactor) {
    if (_list.size() < frameFactor) return false;
