...
void GaitGenerator::setFootRotateState(bool button) {
  ...
  if (button) {
    robot_controller_._robot_data->rotateAngle =
        M_PI / 11. * ratio +
        robot_controller_._robot_data->rotateAngle * (1 - ratio);
  } else {
    robot_controller_._robot_data->rotateAngle =
        M_PI / 22. * ratio +
        robot_controller_._robot_data->rotateAngle * (1 - ratio);
...
void GaitGenerator::FifthPoly(
    Eigen::VectorXd p0, Eigen::VectorXd p0_dot,
...
void GaitGenerator::FifthPoly(
    double p0, double p0_dot, double p0_dotdot, // start point states
    ...
    double &pd, double &pd_dot, double &pd_dotdot) // output command
{
  ...
  if (t < totalTime) {
    ...
    a = A * x0;
    pd = a(0) + a(1) * t + a(2) * t * t + a(3) * t * t * t +
         a(4) * t * t * t * t + a(5) * t * t * t * t * t;
    pd_dot = a(1) + 2 * a(2) * t + 3 * a(3) * t * t + 4 * a(4) * t * t * t +
             5 * a(5) * t * t * t * t;
    pd_dotdot =
        2 * a(2) + 6 * a(3) * t + 12 * a(4) * t * t + 20 * a(5) * t * t * t;
  } else {
...
void GaitGenerator::Thirdpoly(
    double p0, double p0_dot, double p1, double p1_dot,
    double totalTime,   // total permating time
    double currenttime, // current time,from 0 to total time
...
void GaitGenerator::ExcitingTrajectoryFunction(
    Eigen::VectorXd series_para, Eigen::VectorXd init_pos, int dof_num,
  ...
  for (int i = 0; i < dof_num; i++) {
    ...
    for (int j = 1; j < (series_num + 1); j++) {
      pos(i) += series_para(j + 2 * series_num * i - 1) *
                sin(base_ang_freq * j * curtime);
      pos(i) += series_para(j + 2 * series_num * i + series_num - 1) *
                cos(base_ang_freq * j * curtime);

      vel(i) += series_para(j + 2 * series_num * i - 1) *
                cos(base_ang_freq * j * curtime) * base_ang_freq * j;
      vel(i) -= series_para(j + 2 * series_num * i + series_num - 1) *
                sin(base_ang_freq * j * curtime) * base_ang_freq * j;

      acc(i) -= series_para(j + 2 * series_num * i - 1) *
                sin(base_ang_freq * j * curtime) * base_ang_freq *
                base_ang_freq * j * j;
      acc(i) -= series_para(j + 2 * series_num * i + series_num - 1) *
                cos(base_ang_freq * j * curtime) * base_ang_freq *
                base_ang_freq * j * j;
    }
  }
}
...
Eigen::Matrix3d
GaitGenerator::QuanteiniontoMatrix(RigidBodyDynamics::Math::Quaternion Q) {
  double x = Q[0];
  double y = Q[1];
  double z = Q[2];
  double w = Q[3];
...
void GaitGenerator::QuaternionInterp(Eigen::Matrix3d R_start,
                                     Eigen::Matrix3d R_end, double totaltime,
                                     double currenttime, Eigen::Matrix3d &R_d,
                                     Eigen::Vector3d &omiga_d,
