...
public:
  ...
  /// \brief Gets the latest collection of current data received from
  /// asychronous messages from the sensor.
  ///
  ...
  ///
  /// \param[in] timeoutMs The number of milliseconds to wait for the next
  /// available data. \return The next received packet of data. \exception
  /// timeout Did not receive new data by the timeout.
  CompositeData getNextData(int timeoutMs);

private:
  static void asyncPacketReceivedHandler(void *userData,
                                         protocol::uart::Packet &p,
                                         size_t index);
