...
//     cout << i << endl;
//
// HACK: Currently not checking the compiler and defaulting to no range loop
// support.
...
// The VN_SUPPORTS_CSTR_STRING_CONCATENATE define indictes if the compiler
// supports concatenating a C-style string with std::string using the '+'
// operator.
//
// [Example]
//
...
// runtime_error(std::string("Permission denied for item '" + itemName +
// "'.").c_str()) { } #else explicit permission_denied(std::string itemName) :
// runtime_error("Permission denied for item.") { } #endif
//
