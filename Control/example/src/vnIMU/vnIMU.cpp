...
bool vnIMU::initIMU() {
  ...
  vs.registerAsyncPacketReceivedHandler(
      NULL, vnIMU::asciiOrBinaryAsyncMessageReceived);
...
void vnIMU::asciiOrBinaryAsyncMessageReceived(void *userData, Packet &p,
                                              size_t index) {
  ...
  if (p.type() == Packet::TYPE_BINARY) {
    ...
    if (!p.isCompatible(COMMONGROUP_YAWPITCHROLL | COMMONGROUP_ANGULARRATE |
                            COMMONGROUP_ACCEL,
    ...
    imuData[2] = ypr[2] > 0.0
                     ? (ypr[2] / 180.0 - 1.0) * M_PI
                     : (ypr[2] / 180.0 + 1.0) *
                           M_PI; // roll + pi, modify to be in [-pi, pi]
    for (int i = 0; i < 3; ++i) {
      imuData[i + 3] = dypr[i];
      imuData[i + 6] = acc[i];
    }
