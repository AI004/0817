...
void Thread::start(void *routineData) {
  if (_pimpl->StartRoutine != NULL) {
    ...
    _pimpl->ThreadHandle =
        CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)_pimpl->StartRoutine,
                     routineData, 0, NULL);
    ...
    int errorCode = pthread_create(&_pimpl->ThreadHandle, NULL,
                                   Impl::StartRoutineWrapper, _pimpl);
...
void Thread::join() {
  ...
  if (pthread_join(_pimpl->ThreadHandle, NULL))
    throw unknown_error();
