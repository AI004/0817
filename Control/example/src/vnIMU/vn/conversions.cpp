...
vec4f yprInRads2Quat(vec3f ypr) {
  ...
  return vec4f(c1 * c2 * s3 - s1 * s2 * c3, c1 * s2 * c3 + s1 * c2 * s3,
...
mat3f yprInRads2Dcm(vec3f yprInRads) {
  ...
  return mat3f(ct2 * ct1, -ct2 * st1, st2, st3 * st2 * ct1 + ct3 * st1,
               -st3 * st2 * st1 + ct3 * ct1, -st3 * ct2,
               -ct3 * st2 * ct1 + st3 * st1, ct3 * st2 * st1 + st3 * ct1,
               ct3 * ct2);
#else
  return mat3f(ct2 * ct1, ct2 * st1, -st2, st3 * st2 * ct1 - ct3 * st1,
               st3 * st2 * st1 + ct3 * ct1, st3 * ct2,
...
mat3f quat2dcm(vec4f quat) {
  ...
  return mat3f(q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3, 2.f * (q1 * q2 + q0 * q3),
               2.f * (q1 * q3 - q0 * q2), 2.f * (q1 * q2 - q0 * q3),
               q0 * q0 - q1 * q1 + q2 * q2 - q3 * q3, 2.f * (q2 * q3 + q0 * q1),
               2.f * (q1 * q3 + q0 * q2), 2.f * (q2 * q3 - q0 * q1),
...
vec3f dcm2YprInRads(mat3f dcm) {
  return vec3f(atan2(dcm.e01, dcm.e00), asin(-dcm.e02),
...
vec4f dcm2quat(mat3f dcm) {
  ...
  float b2[] = {(1.f + tr) / 4.f, (1.f + 2.f * dcm.e00 - tr) / 4.f,
...
vec3f quat2omegaPhiKappaInRads(vec4f quat) {
  ...
  float omega =
      atanf(-2 * (q1 * q3 - q3 * q4) / (q3 * q3 - q4 * q4 - q1 * q1 + q2 * q2));
  float phi = asinf(2 * (q4 * q2 + q3 * q1));
  float kappa =
      atanf(-2 * (q4 * q1 - q3 * q2) / (q3 * q3 + q4 * q4 - q1 * q1 - q2 * q2));
