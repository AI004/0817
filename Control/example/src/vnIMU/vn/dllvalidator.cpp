...
void dumpImportDirectory32(PeLib::PeFile32 &pef,
                           std::vector<std::string> &dllNamesOut) {
  if (pef.readImportDirectory() == PeLib::ERROR_NO_ERROR) {
    const PeLib::ImportDirectory32 &imp =
        static_cast<PeLib::PeFile32 &>(pef).impDir();
...
void dumpImportDirectory64(PeLib::PeFile64 &pef,
                           std::vector<std::string> &dllNamesOut) {
  if (pef.readImportDirectory() == PeLib::ERROR_NO_ERROR) {
    const PeLib::ImportDirectory64 &imp =
        static_cast<PeLib::PeFile64 &>(pef).impDir();
...
DllValidator::DllValidator(std::string dllName, std::string currentDirectory)
    : mIsInitialized(false), mIsValid(false), mPeFile(NULL), mFileName(dllName),
...
void DllValidator::getMissingDllNames(
    std::vector<std::string> &missingDllNamesOut) {
...
bool DllValidator::validate() {
  ...
  if (mIsInitialized) {
    ...
    for (size_t index = 0; index < mVisitor.mRequiredDlls.size(); index++) {
      HMODULE module = LoadLibraryEx(mVisitor.mRequiredDlls[index].c_str(),
