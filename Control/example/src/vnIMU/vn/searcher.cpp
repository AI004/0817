...
void testValidPacketFoundHandler(void *userData, Packet &packet,
...
vector<uint32_t> TestBaudrates{115200, 128000, 230400, 460800, 921600,
                               57600,  38400,  19200,  9600};
#else
uint32_t TestBaudratesRaw[] = {115200, 128000, 230400, 460800, 921600,
                               57600,  38400,  19200,  9600};
#endif
...
bool Searcher::search(const string &portName, int32_t *foundBaudrate) {
  ...
  vector<uint32_t> TestBaudrates(
      TestBaudratesRaw, TestBaudratesRaw + sizeof(TestBaudratesRaw) /
                                               sizeof(TestBaudratesRaw[0]));
#endif
  for (vector<uint32_t>::const_iterator it = TestBaudrates.begin();
       it != TestBaudrates.end(); ++it) {
...
vector<pair<string, uint32_t>> Searcher::search(vector<string> &portsToCheck) {
  ...
  for (vector<string>::const_iterator it = portsToCheck.begin();
       it != portsToCheck.end(); ++it) {
  ...
  for (list<SearchHelper *>::const_iterator it = helpers.begin();
       it != helpers.end(); ++it) {
    SearchHelper *sh = (*it);
...
bool Searcher::test(string portName, uint32_t baudrate) {
  ...
  try {
    ...
    std::cout << baudrate << " is an invalid baudrate for " << portName
              << std::endl;
    return false;
  } catch (exception e) {
