...
sensor_error::sensor_error(SensorError e)
    : exception(), error(e), _errorMessage(NULL) {}
...
struct VnSensor::Impl {
  ...
  explicit Impl(VnSensor *backReference)
      : pSerialPort(NULL), port(NULL), SimplePortIsOurs(false),
        DidWeOpenSimplePort(false), _rawDataReceivedHandler(NULL),
        _rawDataReceivedUserData(NULL), _possiblePacketFoundHandler(NULL),
        _possiblePacketFoundUserData(NULL), _dataRunningIndex(0),
        _asyncPacketReceivedHandler(NULL), _asyncPacketReceivedUserData(NULL),
        _sendErrorDetectionMode(ERRORDETECTIONMODE_CHECKSUM),
        BackReference(backReference), _waitingForResponse(false),
        _errorPacketReceivedHandler(NULL), _errorPacketReceivedUserData(NULL),
...
#endif
  {
    _packetFinder.registerPossiblePacketFoundHandler(
        this, possiblePacketFoundHandler);
  }

  ~Impl() { _packetFinder.unregisterPossiblePacketFoundHandler(); }

  void onPossiblePacketFound(Packet &possiblePacket,
                             size_t packetStartRunningIndex) {
    if (_possiblePacketFoundHandler != NULL)
      _possiblePacketFoundHandler(_possiblePacketFoundUserData, possiblePacket,
                                  packetStartRunningIndex);
  }
  ...
  void onAsyncPacketReceived(Packet &asciiPacket, size_t runningIndex,
    ...
    if (_asyncPacketReceivedHandler != NULL)
      _asyncPacketReceivedHandler(_asyncPacketReceivedUserData, asciiPacket,
                                  runningIndex);

#if PYTHON
    BackReference->eventAsyncPacketReceived.fire(asciiPacket, runningIndex,
                                                 timestamp);
#endif
    ...
    //
    //	boost::python::call<void>(_asyncPacketReceivedHandlerPython,
    //boost::ref(asciiPacket), runningIndex);
    //}
    //#endif
  }
  ...
  void onErrorPacketReceived(Packet &errorPacket, size_t runningIndex) {
    if (_errorPacketReceivedHandler != NULL)
      _errorPacketReceivedHandler(_errorPacketReceivedUserData, errorPacket,
                                  runningIndex);
  }

  static void possiblePacketFoundHandler(void *userData, Packet &possiblePacket,
                                         size_t packetStartRunningIndex,
                                         TimeStamp timestamp) {
  ...
  static void dataReceivedHandler(void *userData) {
    ...
    pi->port->read(readBuffer, DefaultReadBufferSize, numOfBytesRead);
    ...
    if (pi->_rawDataReceivedHandler != NULL)
      pi->_rawDataReceivedHandler(pi->_rawDataReceivedUserData,
                                  reinterpret_cast<char *>(readBuffer),
                                  numOfBytesRead, pi->_dataRunningIndex);
    ...
    if (pi->_rawDataReceivedHandlerPython != NULL) {
      ...
      boost::python::call<void>(pi->_rawDataReceivedHandlerPython, pRawData,
                                pi->_dataRunningIndex);
    }
#endif

    pi->_packetFinder.processReceivedData(reinterpret_cast<char *>(readBuffer),
                                          numOfBytesRead, t);
...
#if defined(_MSC_VER)
// Unable to use save version 'sprintf_s' since the length of 'toSend' is
// unknown here.
#pragma warning(push)
...
#pragma warning(pop)
  ...
  Packet transactionWithWait(char *toSend, size_t length,
    ...
    while (true) {
      ...
      xplat::Event::WaitResult waitResult = _newResponsesEvent.waitUs(
          static_cast<uint32_t>(responseWaitTime * 1000));
  ...
  void transactionNoFinalize(char *toSend, size_t length, bool waitForReply,
                             Packet *response, uint16_t responseTimeoutMs,
                             uint16_t retransmitDelayMs) {
    if (!isConnected())
      throw invalid_operation();

    if (waitForReply) {
      *response = transactionWithWait(toSend, length, responseTimeoutMs,
                                      retransmitDelayMs);
  ...
  void transactionNoFinalize(char *toSend, size_t length, bool waitForReply,
                             Packet *response) {
    transactionNoFinalize(toSend, length, waitForReply, response,
                          _responseTimeoutMs, _retransmitDelayMs);
  }

  void transaction(char *toSend, size_t length, bool waitForReply,
                   Packet *response, uint16_t responseTimeoutMs,
                   uint16_t retransmitDelayMs) {
    length = finalizeCommandToSend(toSend, length);

    transactionNoFinalize(toSend, length, waitForReply, response,
                          responseTimeoutMs, retransmitDelayMs);
  }
  ...
  void transaction(char *toSend, size_t length, bool waitForReply,
                   Packet *response) {
    transaction(toSend, length, waitForReply, response, _responseTimeoutMs,
                _retransmitDelayMs);
  }
  ...
  BinaryOutputRegister readBinaryOutput(uint8_t binaryOutputNumber) {
    char toSend[17];
    Packet response;
    uint16_t asyncMode, rateDivisor, outputGroup, commonField, timeField,
        imuField, gpsField, attitudeField, insField, gps2Field;

#if VN_HAVE_SECURE_CRT
    int length = sprintf_s(toSend, sizeof(toSend), "$VNRRG,%u*",
                           74 + binaryOutputNumber);
#else
    ...
    response.parseBinaryOutput(&asyncMode, &rateDivisor, &outputGroup,
                               &commonField, &timeField, &imuField, &gpsField,
                               &attitudeField, &insField, &gps2Field);

    return BinaryOutputRegister(
        static_cast<AsyncMode>(asyncMode), rateDivisor,
  ...
  void writeBinaryOutput(uint8_t binaryOutputNumber,
                         BinaryOutputRegister &fields, bool waitForReply) {
    char toSend[256];
    Packet response;
...
#if VN_HAVE_SECURE_CRT
    int length = sprintf_s(toSend, sizeof(toSend), "$VNWRG,%u,%u,%u,%X",
                           74 + binaryOutputNumber, fields.asyncMode,
                           fields.rateDivisor, groups);
#else
    int length = sprintf(toSend, "$VNWRG,%u,%u,%u,%X", 74 + binaryOutputNumber,
                         fields.asyncMode, fields.rateDivisor, groups);
#endif

    if (fields.commonField)
#if VN_HAVE_SECURE_CRT
      length += sprintf_s(toSend + length, sizeof(toSend) - length, ",%X",
                          fields.commonField);
#else
...
vector<uint32_t> VnSensor::supportedBaudrates() {
  uint32_t br[] = {9600,   19200,  38400,  57600, 115200,
...
string VnSensor::transaction(string toSend) {
  ...
  } else if (toSend[toSend.length() - 2] != '\r' &&
...
string VnSensor::send(string toSend, bool waitForReply,
                      ErrorDetectionMode errorDetectionMode) {
  Packet p;
  char *buffer =
      new char[toSend.size() + 8]; // Extra room for possible additions.
  ...
  if (astrickLocation == curToSendLength - 1) {
    if (errorDetectionMode == ERRORDETECTIONMODE_CHECKSUM) {
      ...
      curToSendLength += sprintf_s(
          buffer + curToSendLength, toSend.size() + 8 - curToSendLength,
          "%02X\r\n", Checksum8::compute(buffer + 1, curToSendLength - 2));
#else
      curToSendLength +=
          sprintf(buffer + curToSendLength, "%02X\r\n",
                  Checksum8::compute(buffer + 1, curToSendLength - 2));
#endif
...
#else
      curToSendLength +=
          sprintf(buffer + curToSendLength, "%04X\r\n",
                  Crc16::compute(buffer + 1, curToSendLength - 2));
#endif
    } else {
#if VN_HAVE_SECURE_CRT
      curToSendLength +=
          sprintf_s(buffer + curToSendLength,
                    toSend.size() + 8 - curToSendLength, "*XX\r\n");
#else
  ...
  _pi->transactionNoFinalize(buffer, curToSendLength, waitForReply, &p,
                             _pi->_responseTimeoutMs, _pi->_retransmitDelayMs);
...
void VnSensor::registerRawDataReceivedHandler(void *userData,
...
void VnSensor::writeSettings(bool waitForReply) {
  ...
  size_t length = Packet::genWriteSettings(_pi->_sendErrorDetectionMode, toSend,
                                           sizeof(toSend));
  ...
  // from the sensor.
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response, 2500,
                             1000);
}
...
void VnSensor::tare(bool waitForReply) {
  ...
  size_t length =
      Packet::genTare(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::setGyroBias(bool waitForReply) {
  ...
  size_t length = Packet::genSetGyroBias(_pi->_sendErrorDetectionMode, toSend,
                                         sizeof(toSend));
...
void VnSensor::magneticDisturbancePresent(bool disturbancePresent,
                                          bool waitForReply) {
  char toSend[16];
...
void VnSensor::accelerationDisturbancePresent(bool disturbancePresent,
                                              bool waitForReply) {
  char toSend[16];
...
void VnSensor::restoreFactorySettings(bool waitForReply) {
  ...
  size_t length = Packet::genRestoreFactorySettings(
...
void VnSensor::reset(bool waitForReply) {
  ...
  size_t length =
      Packet::genReset(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeBinaryOutput1(BinaryOutputRegister &fields,
                                  bool waitForReply) {
  _pi->writeBinaryOutput(1, fields, waitForReply);
}

void VnSensor::writeBinaryOutput2(BinaryOutputRegister &fields,
                                  bool waitForReply) {
  _pi->writeBinaryOutput(2, fields, waitForReply);
}

void VnSensor::writeBinaryOutput3(BinaryOutputRegister &fields,
                                  bool waitForReply) {
...
uint32_t VnSensor::readSerialBaudRate(uint8_t port) {
  ...
  size_t length = Packet::genReadSerialBaudRate(_pi->_sendErrorDetectionMode,
                                                toSend, sizeof(toSend), port);
...
void VnSensor::writeSerialBaudRate(const uint32_t &baudrate, uint8_t port,
                                   bool waitForReply) {
  char toSend[25];
...
AsciiAsync VnSensor::readAsyncDataOutputType(uint8_t port) {
  ...
  size_t length = Packet::genReadAsyncDataOutputType(
...
void VnSensor::writeAsyncDataOutputType(AsciiAsync ador, uint8_t port,
                                        bool waitForReply) {
  char toSend[21];
...
void VnSensor::writeAsyncDataOutputFrequency(const uint32_t &adof, uint8_t port,
                                             bool waitForReply) {
  char toSend[26];
...
InsBasicConfigurationRegisterVn200 VnSensor::readInsBasicConfigurationVn200() {
  ...
  size_t length = Packet::genReadInsBasicConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeInsBasicConfigurationVn200(
  ...
  size_t length = Packet::genWriteInsBasicConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.scenario,
      fields.ahrsAiding, 0);
...
void VnSensor::writeInsBasicConfigurationVn200(Scenario scenario,
                                               const uint8_t &ahrsAiding,
                                               bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteInsBasicConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), scenario,
      ahrsAiding, 0);
...
void VnSensor::writeInsBasicConfigurationVn300(
  ...
  size_t length = Packet::genWriteInsBasicConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.scenario,
      fields.ahrsAiding, fields.estBaseline);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeInsBasicConfigurationVn300(Scenario scenario,
                                               const uint8_t &ahrsAiding,
...
string VnSensor::readUserTag() {
  ...
  size_t length = Packet::genReadUserTag(_pi->_sendErrorDetectionMode, toSend,
                                         sizeof(toSend));
...
void VnSensor::writeUserTag(const string &tag, bool waitForReply) {
  ...
  size_t length = Packet::genWriteUserTag(_pi->_sendErrorDetectionMode, toSend,
                                          sizeof(toSend), tag.c_str());
...
string VnSensor::readModelNumber() {
  ...
  size_t length = Packet::genReadModelNumber(_pi->_sendErrorDetectionMode,
...
uint32_t VnSensor::readHardwareRevision() {
  ...
  size_t length = Packet::genReadHardwareRevision(_pi->_sendErrorDetectionMode,
                                                  toSend, sizeof(toSend));
...
uint32_t VnSensor::readSerialNumber() {
  ...
  size_t length = Packet::genReadSerialNumber(_pi->_sendErrorDetectionMode,
                                              toSend, sizeof(toSend));
...
string VnSensor::readFirmwareVersion() {
  ...
  size_t length = Packet::genReadFirmwareVersion(_pi->_sendErrorDetectionMode,
                                                 toSend, sizeof(toSend));
...
void VnSensor::writeSerialBaudRate(const uint32_t &baudrate,
                                   bool waitForReply) {
  char toSend[25];

  size_t length = Packet::genWriteSerialBaudRate(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), baudrate);
...
void VnSensor::writeAsyncDataOutputType(AsciiAsync ador, bool waitForReply) {
  ...
  size_t length = Packet::genWriteAsyncDataOutputType(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), ador);
...
void VnSensor::writeAsyncDataOutputFrequency(const uint32_t &adof,
                                             bool waitForReply) {
  char toSend[26];
...
vec3f VnSensor::readYawPitchRoll() {
  ...
  size_t length = Packet::genReadYawPitchRoll(_pi->_sendErrorDetectionMode,
                                              toSend, sizeof(toSend));
...
vec4f VnSensor::readAttitudeQuaternion() {
  ...
  size_t length = Packet::genReadAttitudeQuaternion(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
vec3f VnSensor::readMagneticMeasurements() {
  ...
  size_t length = Packet::genReadMagneticMeasurements(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
vec3f VnSensor::readAccelerationMeasurements() {
  ...
  size_t length = Packet::genReadAccelerationMeasurements(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
vec3f VnSensor::readAngularRateMeasurements() {
  ...
  size_t length = Packet::genReadAngularRateMeasurements(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
VnSensor::readMagneticAccelerationAndAngularRates() {
  char toSend[17];
  ...
  response.parseMagneticAccelerationAndAngularRates(&reg.mag, &reg.accel,
                                                    &reg.gyro);
...
VnSensor::readMagneticAndGravityReferenceVectors() {
  char toSend[17];
...
void VnSensor::writeMagneticAndGravityReferenceVectors(
  ...
  size_t length = Packet::genWriteMagneticAndGravityReferenceVectors(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.magRef,
      fields.accRef);
...
void VnSensor::writeMagneticAndGravityReferenceVectors(const vec3f &magRef,
                                                       const vec3f &accRef,
                                                       bool waitForReply) {
  char toSend[256];
...
VnSensor::readFilterMeasurementsVarianceParameters() {
  char toSend[17];
...
void VnSensor::writeFilterMeasurementsVarianceParameters(
  ...
  size_t length = Packet::genWriteFilterMeasurementsVarianceParameters(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend),
      fields.angularWalkVariance, fields.angularRateVariance,
      fields.magneticVariance, fields.accelerationVariance);
...
void VnSensor::writeFilterMeasurementsVarianceParameters(
    ...
    const vec3f &magneticVariance, const vec3f &accelerationVariance,
    bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteFilterMeasurementsVarianceParameters(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), angularWalkVariance,
      angularRateVariance, magneticVariance, accelerationVariance);
...
MagnetometerCompensationRegister VnSensor::readMagnetometerCompensation() {
  ...
  size_t length = Packet::genReadMagnetometerCompensation(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeMagnetometerCompensation(const mat3f &c, const vec3f &b,
                                             bool waitForReply) {
  char toSend[256];
...
VnSensor::readFilterActiveTuningParameters() {
  ...
  response.parseFilterActiveTuningParameters(
      &reg.magneticDisturbanceGain, &reg.accelerationDisturbanceGain,
      &reg.magneticDisturbanceMemory, &reg.accelerationDisturbanceMemory);
...
void VnSensor::writeFilterActiveTuningParameters(
  ...
  size_t length = Packet::genWriteFilterActiveTuningParameters(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend),
      fields.magneticDisturbanceGain, fields.accelerationDisturbanceGain,
      fields.magneticDisturbanceMemory, fields.accelerationDisturbanceMemory);
...
void VnSensor::writeFilterActiveTuningParameters(
    ...
    const float &accelerationDisturbanceMemory, bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteFilterActiveTuningParameters(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend),
      magneticDisturbanceGain, accelerationDisturbanceGain,
      magneticDisturbanceMemory, accelerationDisturbanceMemory);
...
AccelerationCompensationRegister VnSensor::readAccelerationCompensation() {
  ...
  size_t length = Packet::genReadAccelerationCompensation(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeAccelerationCompensation(const mat3f &c, const vec3f &b,
                                             bool waitForReply) {
  char toSend[256];
...
mat3f VnSensor::readReferenceFrameRotation() {
  ...
  size_t length = Packet::genReadReferenceFrameRotation(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeReferenceFrameRotation(const mat3f &c, bool waitForReply) {
  ...
  size_t length = Packet::genWriteReferenceFrameRotation(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), c);
...
VnSensor::readCommunicationProtocolControl() {
  char toSend[17];
  ...
  response.parseCommunicationProtocolControl(
      &serialCount, &serialStatus, &spiCount, &spiStatus, &serialChecksum,
      &spiChecksum, &errorMode);
...
void VnSensor::writeCommunicationProtocolControl(
  ...
  size_t length = Packet::genWriteCommunicationProtocolControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.serialCount,
      fields.serialStatus, fields.spiCount, fields.spiStatus,
      fields.serialChecksum, fields.spiChecksum, fields.errorMode);
...
void VnSensor::writeCommunicationProtocolControl(
    CountMode serialCount, StatusMode serialStatus, CountMode spiCount,
    StatusMode spiStatus, ChecksumMode serialChecksum, ChecksumMode spiChecksum,
    ErrorMode errorMode, bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteCommunicationProtocolControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), serialCount,
      serialStatus, spiCount, spiStatus, serialChecksum, spiChecksum,
      errorMode);
...
SynchronizationControlRegister VnSensor::readSynchronizationControl() {
  ...
  size_t length = Packet::genReadSynchronizationControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseSynchronizationControl(
      &syncInMode, &syncInEdge, &reg.syncInSkipFactor, &syncOutMode,
      &syncOutPolarity, &reg.syncOutSkipFactor, &reg.syncOutPulseWidth);
...
void VnSensor::writeSynchronizationControl(
  ...
  size_t length = Packet::genWriteSynchronizationControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.syncInMode,
      fields.syncInEdge, fields.syncInSkipFactor, fields.syncOutMode,
      fields.syncOutPolarity, fields.syncOutSkipFactor,
...
void VnSensor::writeSynchronizationControl(
    SyncInMode syncInMode, SyncInEdge syncInEdge,
    const uint16_t &syncInSkipFactor, SyncOutMode syncOutMode,
    SyncOutPolarity syncOutPolarity, const uint16_t &syncOutSkipFactor,
    const uint32_t &syncOutPulseWidth, bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteSynchronizationControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), syncInMode,
      syncInEdge, syncInSkipFactor, syncOutMode, syncOutPolarity,
      syncOutSkipFactor, syncOutPulseWidth);
...
SynchronizationStatusRegister VnSensor::readSynchronizationStatus() {
  ...
  size_t length = Packet::genReadSynchronizationStatus(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseSynchronizationStatus(&reg.syncInCount, &reg.syncInTime,
                                      &reg.syncOutCount);

  return reg;
}

void VnSensor::writeSynchronizationStatus(SynchronizationStatusRegister &fields,
                                          bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteSynchronizationStatus(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.syncInCount,
      fields.syncInTime, fields.syncOutCount);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeSynchronizationStatus(const uint32_t &syncInCount,
  ...
  size_t length = Packet::genWriteSynchronizationStatus(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), syncInCount,
      syncInTime, syncOutCount);
...
FilterBasicControlRegister VnSensor::readFilterBasicControl() {
  ...
  size_t length = Packet::genReadFilterBasicControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  uint8_t extAccMode;
  uint8_t extGyroMode;
  response.parseFilterBasicControl(&magMode, &extMagMode, &extAccMode,
                                   &extGyroMode, &reg.gyroLimit);
...
void VnSensor::writeFilterBasicControl(FilterBasicControlRegister &fields,
                                       bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteFilterBasicControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.magMode,
      fields.extMagMode, fields.extAccMode, fields.extGyroMode,
      fields.gyroLimit);
...
void VnSensor::writeFilterBasicControl(MagneticMode magMode,
  ...
  size_t length = Packet::genWriteFilterBasicControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), magMode, extMagMode,
      extAccMode, extGyroMode, gyroLimit);
...
VpeBasicControlRegister VnSensor::readVpeBasicControl() {
  ...
  size_t length = Packet::genReadVpeBasicControl(_pi->_sendErrorDetectionMode,
                                                 toSend, sizeof(toSend));
  ...
  uint8_t tuningMode;
  response.parseVpeBasicControl(&enable, &headingMode, &filteringMode,
                                &tuningMode);
...
void VnSensor::writeVpeBasicControl(VpeBasicControlRegister &fields,
                                    bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteVpeBasicControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.enable,
      fields.headingMode, fields.filteringMode, fields.tuningMode);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeVpeBasicControl(VpeEnable enable, HeadingMode headingMode,
                                    VpeMode filteringMode, VpeMode tuningMode,
                                    bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteVpeBasicControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), enable, headingMode,
      filteringMode, tuningMode);
...
VpeMagnetometerBasicTuningRegister VnSensor::readVpeMagnetometerBasicTuning() {
  ...
  size_t length = Packet::genReadVpeMagnetometerBasicTuning(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseVpeMagnetometerBasicTuning(&reg.baseTuning, &reg.adaptiveTuning,
...
void VnSensor::writeVpeMagnetometerBasicTuning(
  ...
  size_t length = Packet::genWriteVpeMagnetometerBasicTuning(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.baseTuning,
      fields.adaptiveTuning, fields.adaptiveFiltering);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeVpeMagnetometerBasicTuning(const vec3f &baseTuning,
  ...
  size_t length = Packet::genWriteVpeMagnetometerBasicTuning(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), baseTuning,
      adaptiveTuning, adaptiveFiltering);
...
VnSensor::readVpeMagnetometerAdvancedTuning() {
  ...
  response.parseVpeMagnetometerAdvancedTuning(
      &reg.minFiltering, &reg.maxFiltering, &reg.maxAdaptRate,
      &reg.disturbanceWindow, &reg.maxTuning);
...
void VnSensor::writeVpeMagnetometerAdvancedTuning(
  ...
  size_t length = Packet::genWriteVpeMagnetometerAdvancedTuning(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.minFiltering,
      fields.maxFiltering, fields.maxAdaptRate, fields.disturbanceWindow,
      fields.maxTuning);
...
void VnSensor::writeVpeMagnetometerAdvancedTuning(
    const vec3f &minFiltering, const vec3f &maxFiltering,
    const float &maxAdaptRate, const float &disturbanceWindow,
    const float &maxTuning, bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteVpeMagnetometerAdvancedTuning(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), minFiltering,
      maxFiltering, maxAdaptRate, disturbanceWindow, maxTuning);
...
VnSensor::readVpeAccelerometerBasicTuning() {
  char toSend[17];
...
void VnSensor::writeVpeAccelerometerBasicTuning(const vec3f &baseTuning,
                                                const vec3f &adaptiveTuning,
                                                const vec3f &adaptiveFiltering,
                                                bool waitForReply) {
  char toSend[256];
...
VnSensor::readVpeAccelerometerAdvancedTuning() {
  char toSend[17];
...
VpeGyroBasicTuningRegister VnSensor::readVpeGyroBasicTuning() {
  ...
  size_t length = Packet::genReadVpeGyroBasicTuning(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseVpeGyroBasicTuning(&reg.angularWalkVariance, &reg.baseTuning,
                                   &reg.adaptiveTuning);

  return reg;
}

void VnSensor::writeVpeGyroBasicTuning(VpeGyroBasicTuningRegister &fields,
                                       bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteVpeGyroBasicTuning(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend),
      fields.angularWalkVariance, fields.baseTuning, fields.adaptiveTuning);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeVpeGyroBasicTuning(const vec3f &angularWalkVariance,
...
vec3f VnSensor::readFilterStartupGyroBias() {
  ...
  size_t length = Packet::genReadFilterStartupGyroBias(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeFilterStartupGyroBias(const vec3f &bias,
                                          bool waitForReply) {
  char toSend[256];
...
VnSensor::readMagnetometerCalibrationControl() {
  char toSend[17];
  ...
  uint8_t hsiMode;
  uint8_t hsiOutput;
  response.parseMagnetometerCalibrationControl(&hsiMode, &hsiOutput,
                                               &reg.convergeRate);
...
void VnSensor::writeMagnetometerCalibrationControl(
  ...
  size_t length = Packet::genWriteMagnetometerCalibrationControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.hsiMode,
      fields.hsiOutput, fields.convergeRate);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeMagnetometerCalibrationControl(HsiMode hsiMode,
                                                   HsiOutput hsiOutput,
  ...
  size_t length = Packet::genWriteMagnetometerCalibrationControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), hsiMode, hsiOutput,
      convergeRate);
...
VnSensor::readCalculatedMagnetometerCalibration() {
  char toSend[17];
...
float VnSensor::readIndoorHeadingModeControl() {
  ...
  size_t length = Packet::genReadIndoorHeadingModeControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeIndoorHeadingModeControl(const float &maxRateError,
                                             bool waitForReply) {
  char toSend[256];
...
void VnSensor::writeVelocityCompensationMeasurement(const vec3f &velocity,
                                                    bool waitForReply) {
  char toSend[256];
...
VnSensor::readVelocityCompensationControl() {
  char toSend[17];
  ...
  uint8_t mode;
  response.parseVelocityCompensationControl(&mode, &reg.velocityTuning,
                                            &reg.rateTuning);
...
void VnSensor::writeVelocityCompensationControl(
  ...
  size_t length = Packet::genWriteVelocityCompensationControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.mode,
      fields.velocityTuning, fields.rateTuning);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeVelocityCompensationControl(VelocityCompensationMode mode,
  ...
  size_t length = Packet::genWriteVelocityCompensationControl(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), mode,
      velocityTuning, rateTuning);
...
VelocityCompensationStatusRegister VnSensor::readVelocityCompensationStatus() {
  ...
  size_t length = Packet::genReadVelocityCompensationStatus(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseVelocityCompensationStatus(&reg.x, &reg.xDot, &reg.accelOffset,
                                           &reg.omega);
...
ImuMeasurementsRegister VnSensor::readImuMeasurements() {
  ...
  size_t length = Packet::genReadImuMeasurements(_pi->_sendErrorDetectionMode,
                                                 toSend, sizeof(toSend));
  ...
  response.parseImuMeasurements(&reg.mag, &reg.accel, &reg.gyro, &reg.temp,
                                &reg.pressure);
...
GpsConfigurationRegister VnSensor::readGpsConfiguration() {
  ...
  size_t length = Packet::genReadGpsConfiguration(_pi->_sendErrorDetectionMode,
                                                  toSend, sizeof(toSend));
  ...
  uint8_t mode;
  uint8_t ppsSource;
  response.parseGpsConfiguration(&mode, &ppsSource);
...
void VnSensor::writeGpsConfiguration(GpsConfigurationRegister &fields,
                                     bool waitForReply) {
  char toSend[256];
...
void VnSensor::writeGpsConfiguration(GpsMode mode, PpsSource ppsSource,
                                     bool waitForReply) {
  char toSend[256];
...
vec3f VnSensor::readGpsAntennaOffset() {
  ...
  size_t length = Packet::genReadGpsAntennaOffset(_pi->_sendErrorDetectionMode,
                                                  toSend, sizeof(toSend));
...
GpsSolutionLlaRegister VnSensor::readGpsSolutionLla() {
  ...
  size_t length = Packet::genReadGpsSolutionLla(_pi->_sendErrorDetectionMode,
                                                toSend, sizeof(toSend));
  ...
  uint8_t gpsFix;
  response.parseGpsSolutionLla(&reg.time, &reg.week, &gpsFix, &reg.numSats,
                               &reg.lla, &reg.nedVel, &reg.nedAcc,
...
GpsSolutionEcefRegister VnSensor::readGpsSolutionEcef() {
  ...
  size_t length = Packet::genReadGpsSolutionEcef(_pi->_sendErrorDetectionMode,
                                                 toSend, sizeof(toSend));
  ...
  uint8_t gpsFix;
  response.parseGpsSolutionEcef(&reg.tow, &reg.week, &gpsFix, &reg.numSats,
                                &reg.position, &reg.velocity, &reg.posAcc,
                                &reg.speedAcc, &reg.timeAcc);
...
InsSolutionLlaRegister VnSensor::readInsSolutionLla() {
  ...
  size_t length = Packet::genReadInsSolutionLla(_pi->_sendErrorDetectionMode,
                                                toSend, sizeof(toSend));
  ...
  response.parseInsSolutionLla(&reg.time, &reg.week, &reg.status,
                               &reg.yawPitchRoll, &reg.position, &reg.nedVel,
                               &reg.attUncertainty, &reg.posUncertainty,
                               &reg.velUncertainty);
...
InsSolutionEcefRegister VnSensor::readInsSolutionEcef() {
  ...
  size_t length = Packet::genReadInsSolutionEcef(_pi->_sendErrorDetectionMode,
                                                 toSend, sizeof(toSend));
  ...
  response.parseInsSolutionEcef(&reg.time, &reg.week, &reg.status,
                                &reg.yawPitchRoll, &reg.position, &reg.velocity,
                                &reg.attUncertainty, &reg.posUncertainty,
                                &reg.velUncertainty);
...
InsAdvancedConfigurationRegister VnSensor::readInsAdvancedConfiguration() {
  ...
  size_t length = Packet::genReadInsAdvancedConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseInsAdvancedConfiguration(
      &useMag, &usePres, &posAtt, &velAtt, &velBias, &useFoam, &reg.gpsCovType,
      &reg.velCount, &reg.velInit, &reg.moveOrigin, &reg.gpsTimeout,
      &reg.deltaLimitPos, &reg.deltaLimitVel, &reg.minPosUncertainty,
...
void VnSensor::writeInsAdvancedConfiguration(
  ...
  size_t length = Packet::genWriteInsAdvancedConfiguration(
      ...
      fields.usePres, fields.posAtt, fields.velAtt, fields.velBias,
      fields.useFoam, fields.gpsCovType, fields.velCount, fields.velInit,
      fields.moveOrigin, fields.gpsTimeout, fields.deltaLimitPos,
...
void VnSensor::writeInsAdvancedConfiguration(
    const uint8_t &useMag, const uint8_t &usePres, const uint8_t &posAtt,
    const uint8_t &velAtt, const uint8_t &velBias, FoamInit useFoam,
    const uint8_t &gpsCovType, const uint8_t &velCount, const float &velInit,
    const float &moveOrigin, const float &gpsTimeout,
    const float &deltaLimitPos, const float &deltaLimitVel,
    const float &minPosUncertainty, const float &minVelUncertainty,
    bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteInsAdvancedConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), useMag, usePres,
      posAtt, velAtt, velBias, useFoam, gpsCovType, velCount, velInit,
      moveOrigin, gpsTimeout, deltaLimitPos, deltaLimitVel, minPosUncertainty,
      minVelUncertainty);
...
InsStateLlaRegister VnSensor::readInsStateLla() {
  ...
  size_t length = Packet::genReadInsStateLla(_pi->_sendErrorDetectionMode,
                                             toSend, sizeof(toSend));
  ...
  response.parseInsStateLla(&reg.yawPitchRoll, &reg.position, &reg.velocity,
                            &reg.accel, &reg.angularRate);
...
InsStateEcefRegister VnSensor::readInsStateEcef() {
  ...
  size_t length = Packet::genReadInsStateEcef(_pi->_sendErrorDetectionMode,
                                              toSend, sizeof(toSend));
  ...
  response.parseInsStateEcef(&reg.yawPitchRoll, &reg.position, &reg.velocity,
                             &reg.accel, &reg.angularRate);
...
StartupFilterBiasEstimateRegister VnSensor::readStartupFilterBiasEstimate() {
  ...
  size_t length = Packet::genReadStartupFilterBiasEstimate(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseStartupFilterBiasEstimate(&reg.gyroBias, &reg.accelBias,
                                          &reg.pressureBias);
...
void VnSensor::writeStartupFilterBiasEstimate(
  ...
  size_t length = Packet::genWriteStartupFilterBiasEstimate(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.gyroBias,
      fields.accelBias, fields.pressureBias);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeStartupFilterBiasEstimate(const vec3f &gyroBias,
  ...
  size_t length = Packet::genWriteStartupFilterBiasEstimate(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), gyroBias, accelBias,
      pressureBias);
...
DeltaThetaAndDeltaVelocityRegister VnSensor::readDeltaThetaAndDeltaVelocity() {
  ...
  size_t length = Packet::genReadDeltaThetaAndDeltaVelocity(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseDeltaThetaAndDeltaVelocity(&reg.deltaTime, &reg.deltaTheta,
                                           &reg.deltaVelocity);
...
VnSensor::readDeltaThetaAndDeltaVelocityConfiguration() {
  ...
  response.parseDeltaThetaAndDeltaVelocityConfiguration(
      &integrationFrame, &gyroCompensation, &accelCompensation);
...
void VnSensor::writeDeltaThetaAndDeltaVelocityConfiguration(
  ...
  size_t length = Packet::genWriteDeltaThetaAndDeltaVelocityConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend),
      fields.integrationFrame, fields.gyroCompensation,
...
void VnSensor::writeDeltaThetaAndDeltaVelocityConfiguration(
    ...
    CompensationMode accelCompensation, bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteDeltaThetaAndDeltaVelocityConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), integrationFrame,
      gyroCompensation, accelCompensation);
...
VnSensor::readReferenceVectorConfiguration() {
  char toSend[17];
  ...
  response.parseReferenceVectorConfiguration(&useMagModel, &useGravityModel,
                                             &reg.recalcThreshold, &reg.year,
                                             &reg.position);
...
void VnSensor::writeReferenceVectorConfiguration(
  ...
  size_t length = Packet::genWriteReferenceVectorConfiguration(
      ...
      fields.useGravityModel, fields.recalcThreshold, fields.year,
      fields.position);
...
void VnSensor::writeReferenceVectorConfiguration(
    const uint8_t &useMagModel, const uint8_t &useGravityModel,
    const uint32_t &recalcThreshold, const float &year, const vec3d &position,
    bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteReferenceVectorConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), useMagModel,
      useGravityModel, recalcThreshold, year, position);
...
GyroCompensationRegister VnSensor::readGyroCompensation() {
  ...
  size_t length = Packet::genReadGyroCompensation(_pi->_sendErrorDetectionMode,
                                                  toSend, sizeof(toSend));
...
void VnSensor::writeGyroCompensation(GyroCompensationRegister &fields,
                                     bool waitForReply) {
  char toSend[256];
...
void VnSensor::writeGyroCompensation(const mat3f &c, const vec3f &b,
                                     bool waitForReply) {
  char toSend[256];
...
ImuFilteringConfigurationRegister VnSensor::readImuFilteringConfiguration() {
  ...
  size_t length = Packet::genReadImuFilteringConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseImuFilteringConfiguration(
      &reg.magWindowSize, &reg.accelWindowSize, &reg.gyroWindowSize,
      &reg.tempWindowSize, &reg.presWindowSize, &magFilterMode,
...
void VnSensor::writeImuFilteringConfiguration(
  ...
  size_t length = Packet::genWriteImuFilteringConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend),
      fields.magWindowSize, fields.accelWindowSize, fields.gyroWindowSize,
      fields.tempWindowSize, fields.presWindowSize, fields.magFilterMode,
      fields.accelFilterMode, fields.gyroFilterMode, fields.tempFilterMode,
...
void VnSensor::writeImuFilteringConfiguration(
    ...
    const uint16_t &presWindowSize, FilterMode magFilterMode,
    FilterMode accelFilterMode, FilterMode gyroFilterMode,
    FilterMode tempFilterMode, FilterMode presFilterMode, bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteImuFilteringConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), magWindowSize,
      accelWindowSize, gyroWindowSize, tempWindowSize, presWindowSize,
      magFilterMode, accelFilterMode, gyroFilterMode, tempFilterMode,
      presFilterMode);
...
GpsCompassBaselineRegister VnSensor::readGpsCompassBaseline() {
  ...
  size_t length = Packet::genReadGpsCompassBaseline(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
...
void VnSensor::writeGpsCompassBaseline(GpsCompassBaselineRegister &fields,
                                       bool waitForReply) {
  char toSend[256];
...
void VnSensor::writeGpsCompassBaseline(const vec3f &position,
                                       const vec3f &uncertainty,
                                       bool waitForReply) {
  char toSend[256];

  size_t length =
      Packet::genWriteGpsCompassBaseline(_pi->_sendErrorDetectionMode, toSend,
                                         sizeof(toSend), position, uncertainty);
...
VnSensor::readGpsCompassEstimatedBaseline() {
  char toSend[17];
  ...
  response.parseGpsCompassEstimatedBaseline(&estBaselineUsed, &reg.numMeas,
                                            &reg.position, &reg.uncertainty);
...
ImuRateConfigurationRegister VnSensor::readImuRateConfiguration() {
  ...
  size_t length = Packet::genReadImuRateConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend));
  ...
  response.parseImuRateConfiguration(&reg.imuRate, &reg.navDivisor,
                                     &reg.filterTargetRate, &reg.filterMinRate);

  return reg;
}

void VnSensor::writeImuRateConfiguration(ImuRateConfigurationRegister &fields,
                                         bool waitForReply) {
  char toSend[256];

  size_t length = Packet::genWriteImuRateConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.imuRate,
      fields.navDivisor, fields.filterTargetRate, fields.filterMinRate);

  Packet response;
  _pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeImuRateConfiguration(const uint16_t &imuRate,
  ...
  size_t length = Packet::genWriteImuRateConfiguration(
      _pi->_sendErrorDetectionMode, toSend, sizeof(toSend), imuRate, navDivisor,
      filterTargetRate, filterMinRate);
