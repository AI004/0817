...
struct CompositeData::Impl {
  ...
  bool hasYawPitchRoll, hasQuaternion, hasDirectionCosineMatrix, hasMagnetic,
      hasMagneticUncompensated, hasMagneticNed, hasMagneticEcef,
      hasAcceleration, hasAccelerationLinearBody, hasAccelerationUncompensated,
      hasAccelerationLinearNed, hasAccelerationLinearEcef, hasAccelerationNed,
      hasAccelerationEcef, hasAngularRate, hasAngularRateUncompensated,
      hasTemperature, hasPressure, hasPositionGpsLla, hasPositionGps2Lla,
      hasPositionGpsEcef, hasPositionGps2Ecef, hasPositionEstimatedLla,
      hasPositionEstimatedEcef, hasVelocityGpsNed, hasVelocityGps2Ned,
      hasVelocityGpsEcef, hasVelocityGps2Ecef, hasVelocityEstimatedNed,
      hasVelocityEstimatedEcef, hasVelocityEstimatedBody, hasDeltaTime,
      hasDeltaTheta, hasDeltaVelocity, hasTimeStartup, hasTimeGps, hasTimeGps2,
      hasTow, hasWeek, hasGpsWeek, hasGps2Week, hasNumSats, hasNumSats2,
      hasTimeSyncIn, hasVpeStatus, hasInsStatus, hasSyncInCnt, hasSyncOutCnt,
      hasTimeStatus, hasTimeGpsPps, hasTimeGps2Pps, hasGpsTow, hasGps2Tow,
      hasTimeUtc, hasTimeUtc2, hasSensSat, hasFix, hasFix2,
      hasPositionUncertaintyGpsNed, hasPositionUncertaintyGps2Ned,
      hasPositionUncertaintyGpsEcef, hasPositionUncertaintyGps2Ecef,
      hasPositionUncertaintyEstimated, hasVelocityUncertaintyGps,
      hasVelocityUncertaintyGps2, hasVelocityUncertaintyEstimated,
      hasTimeUncertainty, hasTimeUncertainty2, hasAttitudeUncertainty,
      hasTimeInfo, hasTimeInfo2, hasDop, hasDop2;
  vec3f yawPitchRoll, magnetic, magneticUncompensated, magneticNed,
      magneticEcef, acceleration, accelerationLinearBody,
      accelerationUncompensated, accelerationLinearNed, accelerationLinearEcef,
      accelerationNed, accelerationEcef, angularRate, angularRateUncompensated,
      velocityGpsNed, velocityGps2Ned, velocityGpsEcef, velocityGps2Ecef,
      velocityEstimatedNed, velocityEstimatedEcef, velocityEstimatedBody,
      deltaTheta, deltaVelocity, positionUncertaintyGpsNed,
      positionUncertaintyGps2Ned, positionUncertaintyGpsEcef,
      positionUncertaintyGps2Ecef, attitudeUncertainty;
  vec3d positionGpsLla, positionGps2Lla, positionGpsEcef, positionGps2Ecef,
      positionEstimatedLla, positionEstimatedEcef;
  vec4f quaternion;
  mat3f directionConsineMatrix;
  float temperature, pressure, deltaTime, positionUncertaintyEstimated,
      velocityUncertaintyGps, velocityUncertaintyGps2,
      velocityUncertaintyEstimated;
  uint64_t timeStartup, timeGps, timeGps2, timeSyncIn, timeGpsPps, timeGps2Pps,
      gpsTow, gps2Tow;
  double tow;
  ...
  Impl &operator=(Impl &aImpl) {
    ...
    mostRecentlyUpdatedAccelerationType =
        aImpl.mostRecentlyUpdatedAccelerationType;
    mostRecentlyUpdatedAngularRateType =
        aImpl.mostRecentlyUpdatedAngularRateType;
    mostRecentlyUpdatedTemperatureType =
        aImpl.mostRecentlyUpdatedTemperatureType;
    ...
    mostRecentlyUpdatedPositionUncertaintyType =
        aImpl.mostRecentlyUpdatedPositionUncertaintyType;
    mostRecentlyUpdatedVelocityUncertaintyType =
        aImpl.mostRecentlyUpdatedVelocityUncertaintyType;
...
CompositeData::CompositeData(const CompositeData &cd) : _i(NULL) {
  (*this) = cd;
}
...
bool CompositeData::hasCourseOverGround() {
  return _i->mostRecentlyUpdatedVelocityType != Impl::CDVEL_None &&
         _i->mostRecentlyUpdatedVelocityType !=
             Impl::CDVEL_EstimatedBody // TODO: Don't have conversion formula
                                       // from body frame to NED frame.
         && _i->mostRecentlyUpdatedVelocityType !=
                Impl::CDVEL_EstimatedEcef // TODO: Don't have conversion formula
                                          // from ECEF frame to NED frame.
         && _i->mostRecentlyUpdatedVelocityType !=
                Impl::CDVEL_GpsEcef // TODO: Don't have conversion formula from
                                    // ECEF frame to NED frame.
         && _i->mostRecentlyUpdatedVelocityType !=
                Impl::CDVEL_Gps2Ecef; // TODO: Don't have conversion formula
                                      // from ECEF frame to NED frame.
}
...
void CompositeData::parseAscii(Packet &p, vector<CompositeData *> &o) {
  ...
  case VNGPS: {
    ...
    p.parseVNGPS(&time, &week, &fix, &numSats, &lla, &nedVel, &nedAcc,
                 &speedAcc, &timeAcc);
    ...
    for (cditer i = o.begin(); i != o.end(); ++i) {
      ...
      // Convert to uint32_t since this is the binary representation in
      // nanoseconds.
  ...
  case VNGPE: {
    ...
    p.parseVNGPE(&tow, &week, &fix, &numSats, &position, &ecefVel, &ecefAcc,
                 &speedAcc, &timeAcc);
  ...
  case VNINS: {
    ...
    p.parseVNINS(&tow, &week, &status, &ypr, &position, &nedVel,
                 &attUncertainty, &posUncertainty, &velUncertainty);
  ...
  case VNINE: {
    ...
    p.parseVNINE(&tow, &week, &status, &ypr, &position, &velocity,
                 &attUncertainty, &posUncertainty, &velUncertainty);
...
void CompositeData::parseBinary(Packet &p, vector<CompositeData *> &o) {
  ...
  if (groups & BINARYGROUP_COMMON)
    parseBinaryPacketCommonGroup(
        p, CommonGroup(p.groupField(curGroupFieldIndex++)), o);
  if (groups & BINARYGROUP_TIME)
    parseBinaryPacketTimeGroup(p, TimeGroup(p.groupField(curGroupFieldIndex++)),
                               o);
  if (groups & BINARYGROUP_IMU)
    parseBinaryPacketImuGroup(p, ImuGroup(p.groupField(curGroupFieldIndex++)),
                              o);
  if (groups & BINARYGROUP_GPS)
    parseBinaryPacketGpsGroup(p, GpsGroup(p.groupField(curGroupFieldIndex++)),
                              o);
  if (groups & BINARYGROUP_ATTITUDE)
    parseBinaryPacketAttitudeGroup(
        p, AttitudeGroup(p.groupField(curGroupFieldIndex++)), o);
  if (groups & BINARYGROUP_INS)
    parseBinaryPacketInsGroup(p, InsGroup(p.groupField(curGroupFieldIndex++)),
                              o);
  if (groups & BINARYGROUP_GPS2)
    parseBinaryPacketGps2Group(p, GpsGroup(p.groupField(curGroupFieldIndex++)),
                               o);
}

void CompositeData::parseBinaryPacketCommonGroup(Packet &p, CommonGroup gf,
                                                 vector<CompositeData *> &o) {
  ...
  if (gf & COMMONGROUP_INSSTATUS) {
    // Don't know if this is a VN-100, VN-200 or VN-300 so we can't know for
    // sure if this is VpeStatus or InsStatus.
...
void CompositeData::parseBinaryPacketTimeGroup(Packet &p, TimeGroup gf,
                                               vector<CompositeData *> &o) {
...
void CompositeData::parseBinaryPacketImuGroup(Packet &p, ImuGroup gf,
                                              vector<CompositeData *> &o) {
...
void CompositeData::parseBinaryPacketGpsGroup(Packet &p, GpsGroup gf,
                                              vector<CompositeData *> &o) {
  if (gf & GPSGROUP_UTC) {
    TimeUtc t;
...
void CompositeData::parseBinaryPacketAttitudeGroup(Packet &p, AttitudeGroup gf,
                                                   vector<CompositeData *> &o) {
...
void CompositeData::parseBinaryPacketInsGroup(Packet &p, InsGroup gf,
                                              vector<CompositeData *> &o) {
...
void CompositeData::parseBinaryPacketGps2Group(Packet &p, GpsGroup gf,
                                               vector<CompositeData *> &o) {
  if (gf & GPSGROUP_UTC) {
    TimeUtc t;
