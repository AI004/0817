...
#error "Unknown System"
  ...
  explicit Impl(SerialPort *backReference)
      :
#if _WIN32
        NumberOfReceiveDataDroppedSections(0), SerialPortHandle(NULL),
#elif __linux__ || __APPLE__ || __CYGWIN__ || __QNXNTO__
        SerialPortHandle(0),
#else
#error "Unknown System"
#endif
        pThreadForHandlingReceivedDataInternally(NULL), Baudrate(0),
        IsOpen(false), _dataReceivedHandler(NULL), _dataReceivedUserData(NULL),
        pSerialPortEventsThread(NULL), ContinueHandlingSerialPortEvents(false),
        ChangingBaudrate(false),
        PurgeFirstDataBytesWhenSerialPortIsFirstOpened(true),
#if PYTHON && !PL156_ORIGINAL && !PL156_FIX_ATTEMPT_1
        ExternalStopRequest(false), ThreadStopped(false),
#endif
        ThreadIsRunning(false), BackReference(backReference),
  ...
  IgnoreError:
    ...
    try {
      ...
      KeepWaiting:
        ...
        error =
            select(SerialPortHandle + 1, &readfs, NULL, NULL, &readWaitTime);
...
#error "Unknown System"
  ...
  void StartSerialPortNotificationsThread() {
    ...
    pSerialPortEventsThread =
        Thread::startNew(HandleSerialPortNotifications, this);
  }
...

  {
    ...
    BackReference->read(buffer, NumberOfBytesToPurgeOnOpeningSerialPort,
                        numOfBytesRead);
  }
  ...
  void open(bool checkAndToggleIsOpenFlag = true) {
    ...
    SerialPortHandle =
        CreateFileA(fullPortName.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
// Set baudrate, 8n1, no modem control, and enable receiving characters.
    ...
    portSettings.c_cc[VMIN] =
        0; // Block on reads until 0 characters are received.
...
SerialPort::SerialPort(const string &portName, uint32_t baudrate)
    : _pi(new Impl(this)) {
...
/// port.
///
/// \return <c>true</c> if this is an FTDI USB serial port; otherwise
/// <c>false</c>.
bool SerialPort_isFtdiUsbSerialPort(string portName) {
  ...
  while (SetupDiEnumDeviceInfo(deviceInfoSet, curDevId, &deviceData)) {
    ...
    if (!SetupDiGetDeviceRegistryProperty(
            deviceInfoSet, &deviceData, SPDRP_FRIENDLYNAME, NULL,
    ...
    TCHAR mfgName[0x100];
    if (!SetupDiGetDeviceRegistryProperty(deviceInfoSet, &deviceData, SPDRP_MFG,
                                          NULL, (PBYTE)mfgName, sizeof(mfgName),
                                          NULL)) {
      SetupDiDestroyDeviceInfoList(deviceInfoSet);
...
HKEY SerialPort_getRegistryKeyForActiveFtdiPort(string portName,
                                                bool isReadOnly) {
  HDEVINFO deviceInfoSet =
      SetupDiGetClassDevs(&GUID_DEVCLASS_PORTS, NULL, NULL, DIGCF_PRESENT);
  ...
  while (SetupDiEnumDeviceInfo(deviceInfoSet, curDevId, &deviceData)) {
    ...
    if (!SetupDiGetDeviceInstanceId(deviceInfoSet, &deviceData,
...
#if VN_HAVE_SECURE_CRT
  _stprintf_s(ftdiKeyPath, sizeof(ftdiKeyPath),
              TEXT("SYSTEM\\CurrentControlSet\\Enum\\%s\\Device Parameters"),
              deviceInstanceId);
#else
  _stprintf(ftdiKeyPath,
            TEXT("SYSTEM\\CurrentControlSet\\Enum\\%s\\Device Parameters"),
            deviceInstanceId);
#endif
  ...
  DWORD result =
      RegOpenKeyEx(HKEY_LOCAL_MACHINE, ftdiKeyPath, 0, accessType, &ftdiKey);
...
bool SerialPort::determineIfPortIsOptimized(string portName) {
  ...
  // Don't know of any optimizations that need to be done for non-Windows
  // systems.
  return true;
  ...
  if (RegQueryValueEx(ftdiKey, TEXT("LatencyTimer"), NULL, NULL,
...
void SerialPort::optimizePort(string portName) {
  ...
  if (RegSetValueEx(ftdiKey, TEXT("LatencyTimer"), 0, REG_DWORD,
...
std::vector<std::string> SerialPort::getPortNames() {
  ...
  error =
      RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("HARDWARE\\DEVICEMAP\\SERIALCOMM"),
                   0, KEY_READ, &serialCommKey);

  if (ERROR_SUCCESS == error) {
    error = RegQueryInfoKey(serialCommKey, NULL, NULL, NULL, &numOfSubkeys,
                            NULL, NULL, &numOfValues, NULL, NULL, NULL, NULL);
  }
  ...
  if (ERROR_SUCCESS == error) {
    for (size_t i = 0; i < numOfValues; i++) {
      ...
      error = RegEnumValue(serialCommKey, i, value, &capacity, NULL, NULL,
      ...
      int convertResult = WideCharToMultiByte(
          CP_ACP, 0, data, dataSize, converted, sizeof(converted), NULL, NULL);
...
void SerialPort::write(const char data[], size_t length) {
  ...
  result =
      GetOverlappedResult(_pi->SerialPortHandle, &overlapped,
                          reinterpret_cast<LPDWORD>(&numOfBytesWritten), true);
...
void SerialPort::read(char dataBuffer[], size_t numOfBytesToRead,
  ...
  BOOL result = ReadFile(_pi->SerialPortHandle, dataBuffer, numOfBytesToRead,
                         NULL, &overlapped);
  ...
  result = GetOverlappedResult(
      _pi->SerialPortHandle, &overlapped,
      reinterpret_cast<LPDWORD>(&numOfBytesActuallyRead), true);
...
void SerialPort::registerDataReceivedHandler(void *userData,
