...
struct BinaryTracker {
  ...
  explicit BinaryTracker(size_t possibleStartIndex, size_t runningDataIndex,
                         TimeStamp timeFound_)
      : possibleStartIndex(possibleStartIndex), groupsPresentFound(false),
        numOfBytesRemainingToHaveAllGroupFields(0),
        numOfBytesRemainingForCompletePacket(0),
        startFoundInProvidedDataBuffer(true),
        runningDataIndexOfStart(runningDataIndex), timeFound(timeFound_) {}
};
...
bool operator==(const BinaryTracker &lhs, const BinaryTracker &rhs) {
  return lhs.possibleStartIndex == rhs.possibleStartIndex &&
         ...
         lhs.numOfBytesRemainingForCompletePacket ==
             rhs.numOfBytesRemainingForCompletePacket;
}
...
struct PacketFinder::Impl {
  ...
  struct AsciiTracker {
    ...
    AsciiTracker()
        : currentlyBuildingAsciiPacket(false), possibleStartOfPacketIndex(0),
          asciiEndChar1Found(false), runningDataIndexOfStart(0) {}
  ...
  list<BinaryTracker>
      _binaryOnDeck; // Collection of possible binary packets we are checking.
  size_t _runningDataIndex; // Used for correlating raw data with where the
                            // packet was found for the end user.
  ...
  explicit Impl(PacketFinder *backReference)
      : _backReference(backReference),
        ...
        _bufferSize(DefaultReceiveBufferSize), _bufferAppendLocation(0),
        _runningDataIndex(0), _possiblePacketFoundUserData(NULL),
  ...
  Impl(PacketFinder *backReference, size_t internalReceiveBufferSize)
      : _backReference(backReference),
        ...
        _bufferSize(internalReceiveBufferSize), _bufferAppendLocation(0),
        _runningDataIndex(0), _possiblePacketFoundUserData(NULL),
        _possiblePacketFoundHandler(NULL) {}
  ...
  void dataReceived(uint8_t data[], size_t length, TimeStamp timestamp) {
    ...
    for (size_t i = 0; i < length; i++, _runningDataIndex++) {
      if (data[i] == AsciiStartChar) {
      ...
      } else if (_asciiOnDeck.currentlyBuildingAsciiPacket &&
                 data[i] == AsciiEndChar1) {
        ...
        if (data[i] == AsciiEndChar2) {
          ...
          size_t runningIndexOfPacketStart =
              _asciiOnDeck.runningDataIndexOfStart;
          ...
          if (asciiStartFoundInProvidedBuffer) {
            ...
            if (_bufferAppendLocation + i < _bufferSize) {
              ...
              startOfAsciiPacket =
                  _buffer + _asciiOnDeck.possibleStartOfPacketIndex;
              packetLength = _bufferAppendLocation + i + 1 -
      ...
      for (list<BinaryTracker>::iterator it = _binaryOnDeck.begin();
           it != _binaryOnDeck.end(); ++it) {
        BinaryTracker &ez = (*it);
        ...
        if (ez.numOfBytesRemainingToHaveAllGroupFields != 0) {
          ...
          if (ez.numOfBytesRemainingToHaveAllGroupFields == 0) {
            ...
            if (ez.startFoundInProvidedDataBuffer) {
              ...
              // Not all of the packet's group is inside the caller's provided
              // buffer.
              ...
              size_t headerLength = _bufferAppendLocation -
                                    ez.possibleStartIndex +
                                    numOfBytesToCopyIntoReceiveBuffer;

              if (_bufferAppendLocation + numOfBytesToCopyIntoReceiveBuffer <
                  _bufferSize) {
                std::memcpy(_buffer + _bufferAppendLocation, data,
                            numOfBytesToCopyIntoReceiveBuffer);
            ...
            if (remainingBytesForCompletePacket >
        ...
        if (ez.numOfBytesRemainingForCompletePacket == 0) {
          ...
          if (ez.startFoundInProvidedDataBuffer) {
            // The binary packet exists completely in the user's provided
            // buffer.
            packetStart = data + ez.possibleStartIndex;
            packetLength = i - ez.possibleStartIndex + 1;
          } else {
            // The packet is split between our receive buffer and the user's
            // buffer.
            size_t numOfBytesToCopyIntoReceiveBuffer = i + 1;
            ...
            if (_bufferAppendLocation + numOfBytesToCopyIntoReceiveBuffer <
              ...
              packetLength =
                  _bufferAppendLocation - ez.possibleStartIndex + i + 1;
            } else {
              // About to overrun our receive buffer!
              invalidPackets.push(ez);
          ...
          if (!p.isValid()) {
            ...
            // Copy data out of the tracking lists since we will be resetting
            // them.
            BinaryTracker bt = ez;
    ...
    if (_asciiOnDeck.currentlyBuildingAsciiPacket &&
        asciiStartFoundInProvidedBuffer) {
      if (_asciiOnDeck.possibleStartOfPacketIndex <
      ...
      _asciiOnDeck.possibleStartOfPacketIndex =
          _bufferAppendLocation + _asciiOnDeck.possibleStartOfPacketIndex -
          dataIndexToStartCopyingFrom;
    }
    ...
    for (list<BinaryTracker>::iterator it = _binaryOnDeck.begin();
      ...
      if ((*it).startFoundInProvidedDataBuffer) {
        ...
        (*it).possibleStartIndex = (*it).possibleStartIndex -
                                   binaryDataMoveOverIndexAdjustment +
                                   _bufferAppendLocation;
      }
    }

    if (_bufferAppendLocation + length - dataIndexToStartCopyingFrom <
        _bufferSize) {
      ...
      std::memcpy(_buffer + _bufferAppendLocation, copyFromStart,
  ...
  void dispatchPacket(Packet &packet, size_t runningDataIndexAtPacketStart,
    ...
    if (_possiblePacketFoundHandler != NULL) {
      _possiblePacketFoundHandler(_possiblePacketFoundUserData, packet,
                                  runningDataIndexAtPacketStart, timestamp);
    }
  }
};
...
// void PacketFinder::register_packet_found_handler(boost::python::object*
// callable)
