...
struct MemoryPort::Impl {
  ...
  explicit Impl(MemoryPort *backReference)
      : IsOpen(false), _dataReceivedHandler(NULL), _dataReceivedUserData(NULL),
        _dataWrittenHandler(NULL), _dataWrittenUserData(NULL),
        DataAvailableForRead(NULL), DataAvailableForReadLength(0),
...
void MemoryPort::read(char dataBuffer[], size_t numOfBytesToRead,
  ...
  copy(_pi->DataAvailableForRead,
       _pi->DataAvailableForRead + _pi->DataAvailableForReadLength, dataBuffer);

  numOfBytesActuallyRead = _pi->DataAvailableForReadLength;
}

void MemoryPort::registerDataReceivedHandler(void *userData,
...
void MemoryPort::registerDataWrittenHandler(void *userData,
