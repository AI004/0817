...
int main (int argc, char* argv[]) {
  ...
  // 从指定文件目录中读取 URDF 模型
  std::string urdf_file = "/home/g/Documents/0817/Control/Sources/urdf/DuckDuck.urdf";  // 替换为你的 URDF 文件路径
  ...
  // 目标末端执行器位置
  Vector3d target_position (0.0175, 0.1674, -0.3087);
  Vector3d target_orientation (0.1202, 0.1990, 0.2020);
  ...
  for (int i = 0; i < 1000; ++i) {
    ...
    Matrix3d current_orientation_matrix = CalcBodyWorldOrientation(*model, Q_current, body_id).transpose();; // 需要转置后与MatLab中计算结果一致
    ...
    switch (meth_select) {
        ...
        case 2:
            ...
            break;
        case 3:
            q_dot = J_left_foot.completeOrthogonalDecomposition().pseudoInverse() * error; // completeOrthogonalDecomposition()伪逆法 alpha = 0.1
            std::cout << "completeOrthogonalDecomposition()伪逆法 q_dot: " << q_dot.transpose() << std::endl;
            break;
        case 4:
            q_dot = (J_left_foot.transpose() * J_left_foot + lambda*lambda * I66).colPivHouseholderQr().solve(J_left_foot.transpose() * error); // 阻尼最小二乘法 alpha = 2.0
