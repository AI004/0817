...
Eigen::VectorXd AccKalmanFilter::mFilter(Eigen::VectorXd sigIn, Eigen::VectorXd aIn, double trust) {
  ...
  // filter_noise =
  // (filter_noise_per*C.transpose()).lu().solve(C*filter_noise_per*C.transpose()
  // + R); estimation_now = estimation_now +
  // filter_noise*(sigIn-C*estimation_now); estimation_noise_std =
  // (Eigen::MatrixXd::Identity(numFilter,numFilter) -
  // filter_noise*C)*filter_noise_per;
