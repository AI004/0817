...
LeggedKalmanFilter::LeggedKalmanFilter(Eigen::VectorXd d_SysNoise, Eigen::VectorXd d_MeaNoise, double dt, int nState,
  ...
  // B.block(0, 0, numInput, numInput) = Eigen::MatrixXd::Identity(numInput,
  // numInput) * dt;

  // C = Eigen::MatrixXd::Zero(numOutput, numState);
  // C.block(0, 0, numState, numState) = Eigen::MatrixXd::Identity(numState,
  // numState); C.block(numState, 0, numState, numState) =
  // Eigen::MatrixXd::Identity(numState, numState);
  ...
  C.block(0, 6, 2, 2) = -Eigen::MatrixXd::Identity(2, 2);
  C.block(3, 3, 3, 3) = Eigen::MatrixXd::Identity(3, 3);
  C.block(6, 0, 3, 3) = Eigen::MatrixXd::Identity(3, 3);
  C.block(6, 8, 2, 2) = -Eigen::MatrixXd::Identity(2, 2);
  C.block(9, 3, 3, 3) = Eigen::MatrixXd::Identity(3, 3);
...
Eigen::VectorXd LeggedKalmanFilter::mFilter(Eigen::VectorXd sigIn, Eigen::VectorXd aIn, Eigen::VectorXd trust) {
  ...
  // R_.block(0, 0, 4, 4) = (1. + (1. - trust(0)) * high_suspect_number) *
