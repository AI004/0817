...
WalkStateEstimate::WalkStateEstimate() {
  ...
  d_SysNoise << 0.01, 0.01, 0.001, 0.001 * 9.8, 0.001 * 9.8, 0.001 * 9.8, 0.002,
      0.002, 0.002, 0.002;
  d_SysNoise.segment(2, 4) = d_SysNoise.segment(2, 4) / 55.0;
  d_MeaNoise << 0.001, 0.001, 0.001, 0.01, 0.01, 0.01, 0.001, 0.001, 0.001,
      0.01, 0.01, 0.01;
  // d_MeaNoise.segment(3, 3) = d_MeaNoise.segment(3, 3) / 55.;
  // d_MeaNoise.segment(9, 3) = d_MeaNoise.segment(9, 3) / 55.;
  leggedKalman =
      new LeggedKalmanFilter(d_SysNoise, d_MeaNoise, 0.0025, 10, 12, 3);
}
...
bool WalkStateEstimate::estWaistPosVelInWorld(Robot_Data *robotdata,
  ...
  case 0: // Left Sole Stance
    posTorso = robotdata->task_card_set[robotdata->body_task_id]
                   ->X_a.block(0, 3, 1, 3)
                   .transpose() -
               robotdata->task_card_set[robotdata->left_foot_id]
                   ->X_a.block(0, 3, 1, 3)
                   .transpose();
    velTorso = robotdata->task_card_set[robotdata->body_task_id]
                   ->X_a.block(1, 3, 1, 3)
                   .transpose() -
               robotdata->task_card_set[robotdata->left_foot_id]
                   ->X_a.block(1, 3, 1, 3)
  ...
  if (filterType == 0) {
    ...
    trust = std::min(
        trust,
        std::min(1.0 + (robotdata->T - robotdata->t) / robotdata->Tc, 1.0));
    trust = std::max(trust, 0.0);
    trust = 1.0;
    aWorld = rotX(robotdata->q_a(3)) * rotY(robotdata->q_a(4)) *
             rotZ(robotdata->q_a(5)) * robotdata->imuAcc;
...
bool WalkStateEstimate::grfEstimating(Robot_Data *robotdata) {
  ...
  J_grf.block(0, 0, 6, 12) =
      robotdata->task_card_set[robotdata->left_foot_id]->jacobi.block(0, 6, 6,
                                                                      12);
  J_grf.block(6, 0, 6, 12) =
      robotdata->task_card_set[robotdata->right_foot_id]->jacobi.block(0, 6, 6,
                                                                       12);
  RigidBodyDynamics::NonlinearEffects(*(robotdata->robot_model), robotdata->q_a,
                                      robotdata->q_dot_a, nonlinear);
  robotdata->grf = J_grf.transpose().colPivHouseholderQr().solve(
      nonlinear.segment(6, 12) - robotdata->tau_a.segment(6, 12));
...
bool WalkStateEstimate::stateMachine(Robot_Data *robotdata) {
  ...
  if ((robotdata->touch_index_pre == 4 &&
       (robotdata->grf(11) - robotdata->grf_pre(11) >= robotdata->grf_lb ||
        robotdata->grf(11) > 0.3 * robotdata->MG) &&
       robotdata->t > robotdata->Td_ratio * robotdata->T + robotdata->Tc) ||
      (robotdata->touch_index_pre > 2 &&
       robotdata->t > robotdata->T + robotdata->Tc - 0.5 * robotdata->dt)) {
  ...
  if (robotdata->touch_index_pre == 1 &&
      robotdata->grf(11) >= robotdata->grf_ub) {
    robotdata->touch_index = 2;
    robotdata->t_ftd = robotdata->t;
  }
  if ((robotdata->touch_index_pre == 2 &&
       (robotdata->grf(5) - robotdata->grf_pre(5) >= robotdata->grf_lb ||
        robotdata->grf(5) > 0.3 * robotdata->MG) &&
       robotdata->t > robotdata->Td_ratio * robotdata->T + robotdata->Tc) ||
  ...
  if (robotdata->touch_index_pre == 3 &&
      robotdata->grf(5) >= robotdata->grf_ub) {
    robotdata->touch_index = 4;
    robotdata->t_ftd = robotdata->t;
  }

  if ((robotdata->touch_index == 4 &&
       fabs(robotdata->grf(7) - robotdata->grf_pre(7)) >= 2.0) ||
      (robotdata->touch_index == 2 &&
       fabs(robotdata->grf(1) - robotdata->grf_pre(1)) >= 2.0)) {
...
bool WalkStateEstimate::generalStateMachine(Robot_Data *robotdata) {
  for (int i = 0; i < 2; ++i) {
    ...
    if (robotdata->st_leg_pre(i) == 0 &&
        ...
        robotdata->t_leg(i) > robotdata->tst_leg(i) +
                                  robotdata->Td_ratio * robotdata->tsw_leg(i)) {
      robotdata->td_leg(i) = 1;
    }
    if (robotdata->st_leg_pre(i) == 0 &&
        robotdata->t_leg(i) > robotdata->tst_leg(i) + robotdata->tsw_leg(i) -
                                  0.5 * robotdata->dt) {
    ...
    if (robotdata->st_leg_pre(i) == 0 &&
      ...
      double dt = robotdata->offset_leg(1 - i) - robotdata->offset_leg(i) > 0
                      ...
                      : robotdata->offset_leg(1 - i) -
                            robotdata->offset_leg(i) + robotdata->tst_leg(i) +
                            robotdata->tsw_leg(i);
    ...
    if ((robotdata->st_leg(i) == 0 &&
         (fabs(robotdata->grf(6 * i + 1) - robotdata->grf_pre(6 * i + 1)) >=
              2.0 ||
          fabs(robotdata->grf(6 * i + 1)) >= 6.0))) {
      if (robotdata->t_leg(i) >
          robotdata->tst_leg(i) + 0.7 * robotdata->tsw_leg(i)) {
    ...
    } else if (robotdata->t_leg(i) < robotdata->tst_leg(i) + robotdata->Tc_leg -
                                         0.5 * robotdata->dt) {
      robotdata->s_leg(i) =
          1.0 -
          (robotdata->t_leg(i) - robotdata->tst_leg(i)) / robotdata->Tc_leg;
    } else {
      robotdata->s_leg(i) = 0.0;
    }
  }
  robotdata->phase_pre = robotdata->phase;
  if (robotdata->phase_pre == 1 && robotdata->st_leg(0) == 1 &&
      robotdata->st_leg(1) == 1) {
    robotdata->phase = 2;
  }
  if (robotdata->phase_pre == 1 && robotdata->st_leg(0) == 0 &&
      robotdata->st_leg(1) == 0) {
    robotdata->phase = 3;
  }
  if (robotdata->phase_pre < 4 && robotdata->st_leg(0) == 1 &&
      robotdata->st_leg(1) == 0) {
    robotdata->phase = 4;
    robotdata->stance_index = 0;
  }
  ...
  if (robotdata->phase_pre == 4 && robotdata->st_leg(0) == 0 &&
      robotdata->st_leg(1) == 0) {
    robotdata->phase = 6;
  }
  if (robotdata->phase_pre > 3 && robotdata->phase_pre < 7 &&
      robotdata->st_leg(0) == 0 && robotdata->st_leg(1) == 1) {
    robotdata->phase = 1;
    robotdata->stance_index = 1;
  }
  return true;
...
bool WalkStateEstimate::estWaistPosVelInWorld(Robot_Data *robotdata) {
  ...
  posTorso_l = robotdata->task_card_set[robotdata->body_task_id]
                   ->X_a.block(0, 3, 1, 3)
                   .transpose() -
               robotdata->task_card_set[robotdata->left_foot_id]
                   ->X_a.block(0, 3, 1, 3)
                   .transpose();
  velTorso_l = robotdata->task_card_set[robotdata->body_task_id]
                   ->X_a.block(1, 3, 1, 3)
                   .transpose() -
               robotdata->task_card_set[robotdata->left_foot_id]
                   ->X_a.block(1, 3, 1, 3)
  ...
  posTorso_r = robotdata->task_card_set[robotdata->body_task_id]
                   ->X_a.block(0, 3, 1, 3)
                   .transpose() -
               robotdata->task_card_set[robotdata->right_foot_id]
                   ->X_a.block(0, 3, 1, 3)
                   .transpose();
  velTorso_r = robotdata->task_card_set[robotdata->body_task_id]
                   ->X_a.block(1, 3, 1, 3)
                   .transpose() -
               robotdata->task_card_set[robotdata->right_foot_id]
                   ->X_a.block(1, 3, 1, 3)
